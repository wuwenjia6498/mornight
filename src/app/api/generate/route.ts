import { NextRequest, NextResponse } from 'next/server';
import solarlunar from 'solarlunar';

interface GenerateRequest {
  type: 'morning' | 'toddler' | 'primary' | 'quote' | 'picturebook';
  dates?: string[];
  count?: number;
  quoteType?: 'morning' | 'toddler' | 'primary';  // 名人名言的应用场景
  pictureBookCategory?: 'minimalist' | 'childview' | 'philosophy' | 'nature';  // 绘本语言的分类
  regenerate_column?: 'morning' | 'toddler' | 'primary';
}

interface GeneratedContent {
  morning_copies: string[];
  toddler_copies: string[];
  primary_copies: string[];
}

// 幼儿段/小学段生成内容结构
interface SegmentContent {
  copies: string[];
  quote_index: number; // 名人名言的索引位置
}

interface ApiResponse {
  date: string;
  context: {
    season: string;
    solarTerm?: string;
    festival?: string;
    month: number;
    day: number;
    weekday: string;
  };
  content: GeneratedContent;
}

// aihubmixmix平台的API配置
const AIHUBMIX_API_URL = process.env.AIHUBMIX_API_URL || 'https://api.aihubmix.com/v1/chat/completions';

// 计算某月"第 n 个星期几"是几号（用于母亲节、父亲节、感恩节等浮动节日）
// targetWeekday: 0=周日, 1=周一, ..., 6=周六；nth: 1=第一个, 2=第二个, ...
function getNthWeekdayOfMonth(year: number, month: number, targetWeekday: number, nth: number): number {
  const firstDay = new Date(year, month - 1, 1);
  const firstDayWeekday = firstDay.getDay();
  const offset = (targetWeekday - firstDayWeekday + 7) % 7;
  return 1 + offset + (nth - 1) * 7;
}

// 获取日期上下文信息
function getDateContext(dateString: string) {
  // 完全避免Date对象的时区问题，直接解析字符串
  const dateParts = dateString.split('-');
  if (dateParts.length !== 3) {
    throw new Error(`无效的日期格式: ${dateString}`);
  }
  
  const year = parseInt(dateParts[0]);
  const month = parseInt(dateParts[1]);
  const day = parseInt(dateParts[2]);
  
  // 验证日期有效性
  if (isNaN(year) || isNaN(month) || isNaN(day) || 
      month < 1 || month > 12 || day < 1 || day > 31) {
    throw new Error(`无效的日期值: ${dateString}`);
  }
  
  // 仅用于计算星期几，使用本地时间
  const dateForWeekday = new Date(year, month - 1, day);
  const weekdays = ['周日', '周一', '周二', '周三', '周四', '周五', '周六'];
  const weekday = weekdays[dateForWeekday.getDay()];

  // 简单的季节判断
  let season = '';
  if (month >= 3 && month <= 5) season = '春季';
  else if (month >= 6 && month <= 8) season = '夏季';
  else if (month >= 9 && month <= 11) season = '秋季';
  else season = '冬季';

  // 简单的节气判断
  let solarTerm;
  if (month === 1 && day >= 5 && day <= 7) solarTerm = '小寒';
  else if (month === 1 && day >= 20 && day <= 22) solarTerm = '大寒';
  else if (month === 2 && day >= 3 && day <= 5) solarTerm = '立春';
  else if (month === 3 && day >= 20 && day <= 22) solarTerm = '春分';
  else if (month === 6 && day >= 21 && day <= 22) solarTerm = '夏至';
  else if (month === 9 && day >= 22 && day <= 24) solarTerm = '秋分';
  else if (month === 12 && day >= 21 && day <= 23) solarTerm = '冬至';

  // 使用 solarlunar 库获取农历和传统节日
  let lunarData;
  try {
    lunarData = solarlunar.solar2lunar(year, month, day);
  } catch (lunarError) {
    console.error('solarlunar处理失败:', lunarError);
    // 如果农历转换失败，使用空数据继续
    lunarData = { lMonth: 0, lDay: 0, lunarFestival: '', term: '' };
  }
  let festival = '';

  // 优先判断公历节日（固定日期）
  if (month === 1 && day === 1) festival = '元旦';
  else if (month === 2 && day === 14) festival = '情人节';
  else if (month === 3 && day === 8) festival = '妇女节';
  else if (month === 3 && day === 12) festival = '植树节';
  else if (month === 4 && day === 1) festival = '愚人节';
  else if (month === 4 && day === 23) festival = '世界读书日';
  else if (month === 5 && day === 1) festival = '劳动节';
  else if (month === 5 && day === 4) festival = '青年节';
  else if (month === 6 && day === 1) festival = '儿童节';
  else if (month === 9 && day === 10) festival = '教师节';
  else if (month === 10 && day === 1) festival = '国庆节';
  else if (month === 10 && day === 31) festival = '万圣节';
  else if (month === 12 && day === 25) festival = '圣诞节';
  // 公历浮动日期节日
  else if (month === 5 && day === getNthWeekdayOfMonth(year, 5, 0, 2)) festival = '母亲节';
  else if (month === 6 && day === getNthWeekdayOfMonth(year, 6, 0, 3)) festival = '父亲节';
  else if (month === 11 && day === getNthWeekdayOfMonth(year, 11, 4, 4)) festival = '感恩节';
  // 判断农历节日
  else if (lunarData.lMonth === 1 && lunarData.lDay === 1) festival = '春节';
  else if (lunarData.lMonth === 1 && lunarData.lDay === 15) festival = '元宵节';
  else if (lunarData.lMonth === 5 && lunarData.lDay === 5) festival = '端午节';
  else if (lunarData.lMonth === 7 && lunarData.lDay === 7) festival = '七夕节';
  else if (lunarData.lMonth === 8 && lunarData.lDay === 15) festival = '中秋节';
  else if (lunarData.lMonth === 9 && lunarData.lDay === 9) festival = '重阳节';
  else if (lunarData.lMonth === 12 && lunarData.lDay === 8) festival = '腊八节';
  // 如果没有找到节日，再检查solarlunar库返回的节日
  else if (lunarData.lunarFestival) {
    festival = lunarData.lunarFestival;
  }
  // 最后，检查是否是节气
  else if (lunarData.term) {
    solarTerm = lunarData.term;
  }
  
  const result = {
    season,
    solarTerm,
    festival,
    month,
    day,
    weekday,
    // 添加格式化的日期字符串
    formattedDate: `${year}年${month}月${day}日`
  };
  
  return result;
}

// 共享的段落提示词生成函数
function createSegmentSectionPrompt(type: 'toddler' | 'primary', count: number) {
  // 增加写作风格的随机性
  const toddlerTones = [
    { mood: '轻松自然', hint: '语气松弛，像随口聊起一个有意思的事。不端着，不说教，自然流畅' },
    { mood: '温柔笃定', hint: '语气柔软但有力量，像一句说到心坎里的话。不犹豫、不啰嗦' },
    { mood: '好奇发现', hint: '带着"你知道吗"的发现感，分享一个关于阅读的小洞察，让人眼前一亮' },
    { mood: '安静沉淀', hint: '语气安静、有留白，像读完一本好书后的片刻沉思。不急着给答案' }
  ];
  
  const primaryTones = [
    { mood: '平等对话', hint: '不是在指导谁，就是平等地说一个想法。"你有没有注意到……""其实可以换个思路……"' },
    { mood: '一针见血', hint: '不绕弯子，直接点出一个容易被忽略的关键点。简洁有力，读完让人"哦！"' },
    { mood: '温和建议', hint: '用"可以试试""也许""不妨"这样的语气，给建议但不强加。读者觉得被尊重而不是被教育' },
    { mood: '换个角度', hint: '提供一个不一样的视角，让人重新看待一件习以为常的事。"其实换个角度想……"' }
  ];
  
  // 幼儿段方法论体系（16本书合并为8个差异化体系，每体系10个方法）
  const toddlerMethodSystems = [
    {
      source: '绘本的本质与情感（松居直《幸福的种子》《我的图画书论》《绘本之力》/ 林美琴《绘本有什么了不起？》）',
      focus: '绘本不是教材，是情感体验——亲子共读的本质是爱的传递',
      methods: [
        '绘本不是用来"教"的：很多家长读完一本绘本就问"你学到了什么？"——这恰恰是最扫兴的。绘本的首要功能是让孩子感受到快乐、温暖、好奇，不是灌输道理',
        '用耳朵"读"书：松居直说，对幼儿来说，"读"书其实是"听"书。孩子听到的不只是故事，还有你的声音、你的呼吸、你的体温。这些加在一起，才是完整的阅读体验',
        '绘本是大人和孩子之间的桥梁：平时不知道怎么跟孩子聊感受？绘本是最好的中间人。读到小兔子害怕黑暗，孩子可能会说"我也怕"——对话就自然地开始了',
        '反复读同一本书是正常的：孩子反复要求"再讲一遍"不是没记住，是因为这本书让他感到安全和快乐。重复是幼儿确认世界的方式，每一遍他都在获取不同的东西',
        '不要急着"升级"：孩子一直读"简单"的绘本？别着急换难的。松居直说，绘本没有难易之分，只有"孩子现在需要什么"。他需要的就是最好的',
        '亲子共读不是为了识字：把共读变成识字课，是最快消灭阅读兴趣的方法。在这个阶段，让孩子爱上"和你一起看书的感觉"，比认识多少字重要得多',
        '好的绘本大人也会被打动：如果你觉得一本绘本"幼稚"、"没什么内容"，可能是你还没有读到它真正想说的话。好的绘本是大人和孩子可以一起感动的',
        '情感先于认知：0-6岁的孩子，情感发展比认知发展更重要。绘本最大的价值不是教孩子认字、学知识，而是帮他建立安全感、同理心和对世界的信任',
        '每一本绘本都是一颗种子：松居直说，幼年的阅读体验就是播下的种子，不会马上发芽，但总有一天你会发现——那些故事已经在孩子心里生了根',
        '放下"有用"的执念：不是每本绘本都要有教育意义。有些书就是好玩、好笑、好看。快乐本身就是最大的意义'
      ]
    },
    {
      source: '怎么看图画书（彭懿《图画书应该这样读》《世界图画书阅读与经典》/ 郝广才《好绘本如何好？》）',
      focus: '绘本是图文合奏，学会"看图"才算真的会读绘本',
      methods: [
        '文字只讲了一半的故事：很多家长读绘本只读文字，把图画当插图。其实绘本的图画在讲另一半故事——有时候图和文字说的甚至不是同一件事，那才是最有趣的地方',
        '翻页的节奏感：为什么有的绘本让人想快快翻到下一页？因为作者在翻页处制造了悬念。和孩子一起感受翻页前的"然后呢？"——这就是绘本独有的叙事魅力',
        '封面和封底也是故事的一部分：很多家长翻开书就开始读正文。试试和孩子先看封面——"你觉得这会是一个什么样的故事？"封底也别跳过，有些绘本的结局藏在封底里',
        '留白的力量：好的绘本会留空间让你想象。画面里没有画出来的东西，有时候比画出来的更重要。问问孩子"你觉得画面外面还发生了什么？"',
        '色彩是有情绪的：暖色调代表温暖、安全，冷色调代表孤独、紧张。不需要和孩子讲这些理论，但可以问"这一页的颜色让你感觉怎么样？"孩子的感受力比你想象的强',
        '大图和小图的秘密：为什么有的画面占了整整两页，有的却缩在角落？画面越大，情感越强烈；画面越小，角色可能越孤独。这些是图画书的"视觉语法"',
        '视角会说话：画面是从上往下看的？角色会显得渺小。从下往上看？角色会显得强大。和孩子一起发现"画家是站在哪里画的"，这是一个有趣的游戏',
        '文图的配合与反差：有时候文字说"他一点也不害怕"，但图上画的是一个在发抖的小人。这种反差就是幽默的来源。帮孩子发现这种反差，他会笑得合不拢嘴',
        '环衬也有戏：翻开封面后那张"纯色页"（环衬），很多人直接跳过。但好的绘本里，环衬的颜色和图案是有含义的，甚至前后环衬不一样——故事在你翻开之前就已经开始了',
        '一本绘本值得反复"看"：第一遍跟着故事走，第二遍去看图画里的细节，第三遍发现图文之间的秘密。同一本绘本，"看"的方式不同，发现完全不同'
      ]
    },
    {
      source: '选书与经典（松居直《松居直喜欢的50本图画书》/ 彭懿《世界图画书阅读与经典》）',
      focus: '怎么给孩子选书——不是越贵越好、越新越好，关键是适合',
      methods: [
        '孩子自己选的书最好：书店里让孩子自己挑，哪怕他选了一本你觉得"太简单"的。自己选的书他愿意读，你选的书他可能连翻都不翻',
        '不必追求"书单"：网上的"必读书单"是参考，不是标准。每个孩子的兴趣点不同，强迫读他不感兴趣的"经典"，效果还不如他自己挑的一本恐龙书',
        '经典绘本之所以经典：不是因为大人觉得好，而是几十年来无数孩子都喜欢。《母鸡萝丝去散步》简单吗？简单。但它的图文反差让每一代孩子都笑出声——这就是经典的力量',
        '分龄只是参考：标着"3-6岁"的绘本，2岁的孩子可能也爱听，7岁的孩子可能还在读。不要被年龄标签限制住，孩子的反应才是最好的判断标准',
        '同一位作者可以串着读：孩子喜欢《猜猜我有多爱你》？可以找同一个作者的其他作品。熟悉的风格会让孩子有安全感，同时也在拓展阅读面',
        '绘本不只是"故事书"：无字绘本、概念绘本、科普绘本、诗歌绘本——绘本的类型比你想象的多。偶尔换换类型，孩子可能会打开新世界',
        '不要只买不读：家里绘本堆成山，但很多只翻过一遍？挑出十本，轮流读，反复读。十本读透了，比一百本翻过去有价值',
        '图书馆是最好的试衣间：不确定孩子喜不喜欢？先去图书馆借。借回来他抱着不撒手的，再买回家。这是最经济也最准确的选书方式',
        '关注获奖作品但不迷信：凯迪克奖、格林威奖的获奖作品通常水准很高，是选书的好起点。但获奖不代表你家孩子一定喜欢，还是要观察他的反应',
        '跟着孩子的兴趣走：孩子最近迷恐龙？那就借一堆恐龙绘本。迷月亮？那就找关于月亮的书。兴趣是最好的领路人，顺着它走，阅读自然就发生了'
      ]
    },
    {
      source: '亲子共读实操（粲然《骑鲸之旅》1&2 / 汪培珽《喂故事书长大的孩子》）',
      focus: '从0岁开始的共读实操——那些"怎么办"的真实问题',
      methods: [
        '撕书、啃书是正常的：一岁以内的宝宝用嘴巴和手探索世界，书也不例外。撕了也没关系，这说明他在和书互动。这个阶段，布书、纸板书是好选择',
        '坐不住就别强求坐着听：谁规定读书一定要安安静静坐好？孩子在地上爬着、玩着积木，你在旁边读，他听到了——这也是共读。不要把"坐好听"当成前提条件',
        '读书不要变成任务：每天固定要读几本？读完要回答问题？一旦有了KPI，共读就变成了负担。随性一点，想读就读，哪怕只读了两页也很好',
        '从每天五分钟开始：不需要一开始就读半小时。每天五分钟，坚持下来，比偶尔读一次两小时有用得多。习惯是积累出来的',
        '抱着读是最好的姿势：让孩子窝在你怀里，书摊开在面前。他看到的是书，感受到的是你的体温。这个姿势本身就是安全感',
        '孩子打断你是好事：读到一半孩子突然指着图说"狗狗！"——这不是捣乱，是他在参与。停下来，回应他，和他聊两句，然后再继续。互动比读完更重要',
        '不要用绘本"教育"孩子：孩子不刷牙就读刷牙的绘本，孩子打人就读关于友善的绘本——绘本不是行为矫正工具。孩子能感觉到你的目的性，反而会抗拒',
        '爸爸也要参与共读：爸爸读书的声音、节奏和妈妈不同，孩子能从中获得不一样的体验。而且爸爸参与阅读，对孩子阅读兴趣的影响比很多人想象的大',
        '每个孩子的共读节奏不同：有的孩子一岁就能安静听完一本，有的三岁还坐不住。这不是落后，是节奏不同。尊重自己孩子的节奏，别和别人家比',
        '读绘本不需要演技：不必每次都声情并茂、一人分饰多角。用你自然的、舒服的方式读就好。孩子在意的不是你演得好不好，而是你在不在身边'
      ]
    },
    {
      source: '共读中的对话与讨论（钱伯斯《说来听听》/ 佩利《共读绘本的一年》）',
      focus: '读完之后怎么聊——好的对话让一本书活起来',
      methods: [
        '最好的开场白是"你觉得呢"：读完不要急着问"这个故事讲了什么"。换成"你觉得呢？"或者"你最喜欢哪里？"——开放的问题才能打开孩子的话匣子',
        '先说感受，再说理解：钱伯斯建议从三个层次聊——"喜欢的地方""不喜欢的地方""困惑的地方"。不要跳过感受直接问道理，感受是思考的入口',
        '接住孩子的回答，不要评判：孩子说"我觉得大灰狼也很可怜"——别急着纠正。说一句"为什么这样想？"继续听下去。每一个"奇怪"的回答背后，都有孩子自己的逻辑',
        '沉默也是回应的一部分：问了一个问题孩子不说话？别催。给他时间想。有时候十几秒的沉默之后，孩子会说出让你惊讶的话',
        '讨论不是考试：如果孩子觉得每次读完书都要"回答问题"，他会开始害怕读书。讨论应该像聊天一样自然，不想说的时候也可以不说',
        '你的感受也可以说：不只是问孩子"你觉得呢"，你也可以说"这一页我觉得好美"或者"这个结局我没想到"。当大人也分享感受的时候，孩子会更愿意开口',
        '追问比提问更重要：第一个问题打开话题，后面的追问才是深入的关键。"然后呢？""你怎么想到的？""如果换成你呢？"——一步步引向更深的思考',
        '聊着聊着跑题了也没关系：从绘本里的兔子聊到了幼儿园的小兔子，又聊到了今天中午吃的胡萝卜——这不是跑题，这是孩子在做"联结"。他正在用自己的方式理解世界',
        '不是每本书都需要讨论：有些书读完就让它安静地待着就好。过度讨论和不讨论一样，都会消磨乐趣。观察孩子——他想聊的时候自然会开口',
        '佩利的发现——孩子的讨论能力超乎想象：佩利在幼儿园里用一整年的共读实践证明，即使是3-5岁的孩子，也能进行深度的故事讨论，关键是大人愿不愿意认真倾听'
      ]
    },
    {
      source: '朗读的力量（吉姆·崔利斯《朗读手册》）',
      focus: '朗读是给孩子的最好礼物——简单、免费、效果深远',
      methods: [
        '每天朗读十五分钟：不需要很长时间，十五分钟就够了。但关键是每天都有。这十五分钟里，孩子获得的不只是故事，还有专注力、词汇量和安全感',
        '朗读从出生就可以开始：新生儿听不懂故事？没关系。他听到的是你的声音和节奏。崔利斯说，朗读的起点不是"孩子能听懂"，而是"你开始读"',
        '听得懂的比读得懂的多：3岁孩子可能只认识几个字，但他能听懂比自己阅读能力高得多的故事。朗读就是在帮孩子的理解力"坐电梯"，远远跑在识字前面',
        '朗读的仪式感很重要：固定的时间、固定的位置、固定的一盏灯。不是仪式有什么魔力，而是"确定性"给孩子安全感。当睡前故事变成"我们家的规矩"，它就不会被轻易放弃',
        '不要把朗读变成识字课：指着字一个一个教孩子认，或者读到一半停下来问"这个字念什么"——这是在把朗读时光变成上课。孩子要的是故事，不是生字表',
        '章节书也可以朗读：绘本适合朗读，章节书也可以。每天读一章，故事线拉长，孩子的期待感更强。"今晚还读吗？""当然！"——这就是阅读习惯的雏形',
        '你的声音就是最好的有声书：手机里的故事音频再好听，也比不过你的声音。因为你会停顿、会看孩子的反应、会根据他的情绪调整节奏，这是任何音频做不到的',
        '朗读不要停在孩子会自主阅读之后：很多家长觉得孩子自己能读了就不用朗读了。其实不是。孩子自己读的书和听你读的书可以是不同层次的——朗读可以一直持续到小学',
        '读得不好也没关系：不会配音、不会演戏、普通话不标准？没关系。真诚地读，认真地读，你的投入孩子感受得到。完美的技巧远不如真实的陪伴',
        '有时候，一本书改变一切：崔利斯记录了无数家庭因为一本对的书、一段朗读时光，让一个不爱读书的孩子爱上了阅读。改变不一定需要宏大的计划，有时候只需要一本书和一个愿意读的大人'
      ]
    },
    {
      source: '阅读环境与氛围（钱伯斯《打造儿童阅读环境》/ 克拉生《阅读的力量》）',
      focus: '环境的力量远大于说教——让书成为空气一样自然的存在',
      methods: [
        '家里到处都是书：客厅、卧室、餐桌旁、甚至卫生间。让孩子随手都能拿到书，比对他说"去读书"有用十倍。看得见书的孩子，才会拿起书',
        '书架放低一点：把书放在孩子够得着的高度，封面朝外。他能看到什么书，比家里有多少书更重要。一个低矮的、封面朝外的小书架，比顶天立地的大书柜对孩子更友好',
        '有一个属于他的阅读角：不需要多大，一个角落、一个软垫、一盏小灯就够了。当孩子有了"我的读书地方"，阅读就多了一层归属感',
        '大人自己也在读书：这一条比所有方法都重要。如果孩子看到的是你在刷手机，你说一百遍"读书好"也没用。但如果他看到你窝在沙发上看书——不用说话，他已经在学了',
        '自由阅读的时间：克拉生说，给孩子一段完全自由的阅读时间——不考试、不评价、不要求。就是安安静静地各读各的。这段时间的价值，比任何阅读课都大',
        '把书变成日常的一部分：去超市路过书店，进去逛逛；下雨天不知道干嘛，拿本书出来；等餐的时候，包里掏出一本绘本。当阅读融入日常，它就不再是"要完成的任务"了',
        '睡前故事是最强的阅读仪式：每天临睡前的那段故事时间，是很多孩子最期待的时刻。不要轻易取消它，也不要把它当作"表现好才有"的奖赏',
        '别在阅读环境里放电子产品：阅读角旁边如果放着iPad，书的吸引力永远比不过屏幕。不是不让孩子用电子产品，而是给阅读留一个"不被打扰"的空间',
        '兴趣比"该读什么"重要：孩子只爱看车的绘本？没关系。只爱看恐龙？也没关系。克拉生的研究表明，入口不重要，重要的是他在读、他快乐。兴趣会自己扩展',
        '图书馆是最好的阅读环境：定期带孩子去图书馆，让他自己选书、自己翻。图书馆的氛围本身就在说：书是好东西，读书是一件自然的事'
      ]
    },
    {
      source: '阅读身份与内驱力（米勒《书语者》/ 克拉生《阅读的力量》）',
      focus: '让孩子觉得"我是一个爱读书的人"比读了多少本更重要',
      methods: [
        '阅读身份比阅读技巧重要：一个觉得"我是爱读书的人"的孩子，会自己去找书读。一个被逼着读书的孩子，离开监督就不读了。目标不是"让他读"，而是"让他想读"',
        '选择权是内驱力的起点：让孩子自己选书——哪怕选了你觉得"没营养"的。当他感受到"我可以决定读什么"，阅读就从被动变成了主动',
        '弃书权同样重要：读了二十页不想读了？放下就好。大人看剧也会弃剧，为什么要求孩子每本书都读完？保护阅读的胃口比读完任何一本书都重要',
        '记录带来成就感：不需要写读后感，就是简单地记下"今天读了什么"。一个月后翻看记录——"原来我读了这么多！"这种成就感是阅读习惯最好的燃料',
        '不要用阅读作为奖惩工具："你表现好才给你读故事"/"不听话就没有睡前故事了"——这会让孩子觉得阅读是"好处"而不是"日常"。阅读应该像吃饭喝水一样自然',
        '夸对地方：不要夸"你真棒，读了一本书"。可以说"你注意到了一个很有趣的细节"或者"你这个问题问得真好"。关注过程和思考，而不是读了多少',
        '同伴的力量：如果幼儿园的好朋友也在读某本书，孩子的兴趣会翻倍。可以和其他家庭一起共读，或者让孩子给弟弟妹妹"讲书"——角色转换会带来全新的动力',
        '不和别人比：别人家的孩子三岁认字一千个，你家孩子三岁还在撕书——没关系。每个孩子走向阅读的路径不同，快慢不重要，方向对就行',
        '阅读不是只有"正经书"才算：翻翻书、洞洞书、立体书、甚至超市的宣传单——对幼儿来说，一切与"翻看""指认""讨论"相关的行为，都是阅读的一部分',
        '播下种子，耐心等待：有的孩子两岁就爱上书，有的五岁才开始对书感兴趣。米勒说，关键不是孩子什么时候爱上阅读，而是在他准备好之前，有没有人一直在旁边默默地读给他听'
      ]
    }
  ];
  const randomToddlerSystem = toddlerMethodSystems[Math.floor(Math.random() * toddlerMethodSystems.length)];
  const randomToddlerMethod = randomToddlerSystem.methods[Math.floor(Math.random() * randomToddlerSystem.methods.length)];

  // 小学段方法论体系（多本阅读指导书，按核心理念分组）
  const readingMethodSystems = [
    {
      source: '《小学生如何阅读一本小说》',
      focus: '用具体策略拆解一本书，让孩子从"读过"变成"读懂"',
      methods: [
        '预测法：翻开书之前先看封面和标题，猜猜"这会是一个什么样的故事？"读到一半停下来，猜猜"接下来会怎样？"猜对了开心，猜错了更有趣——原来作者比我还会编',
        '提问法：读的时候随时冒出"为什么"就对了。"为什么主角非要去冒险？""作者为什么把这一段写得这么慢？"每一个问题，都是孩子在和书真正对话',
        '联结法：读到主角搬家很难过，就问孩子"你转学那次是不是也这样？"把书里的事和自己的经历连起来，书就不再是"别人的故事"了，变成了"我也懂"的共鸣',
        '画面感：读到"森林里雾蒙蒙的"，停下来闭上眼——你看到什么颜色？闻到什么味道？脚踩在什么上面？文字变成脑海里的电影，故事就活了，也记得住了',
        '人物追踪：拿张纸，把主角在故事开头、中间、结尾的样子画下来或写下来。你会发现，最好的故事里，主角一定是变了的——胆小变勇敢，自私变善良。这个变化，就是故事的灵魂',
        '情节地图：故事就像爬山——起因是出发，发展是上坡，高潮是山顶，结局是下山。让孩子画出这座"山"，故事的骨架一下就看清了，概括能力也练了',
        '词汇侦探：遇到不认识的词不要急着查字典，先根据上下文猜猜什么意思。猜的过程本身就是在理解，这种"从语境中推断词义"的能力，比背词汇表有用多了',
        '比较阅读：同一个作者的两本书，或者同一个主题的两本书，对比着读——风格有什么不同？观点有什么差异？比较的过程中，阅读从"接收"变成了"思辨"',
        '批注法：读到有感触的地方就在旁边写两个字——"感动""不信""好笑""为什么"。不需要写完整的句子，这些碎片化的标记就是思考的痕迹，回头翻看会发现自己的阅读轨迹',
        '重读的发现：第一遍读是被情节拉着走——"后来呢？后来呢？"第二遍读才注意到那些被忽略的细节——"原来第一章就埋了伏笔"。一本好书值得读两遍以上，每次都有新收获'
      ]
    },
    {
      source: '《朗读手册》（吉姆·崔利斯）',
      focus: '朗读是最简单、最直接、最有效的教育方式',
      methods: [
        '亲子朗读：每天给孩子读十五分钟书，比花一万块报阅读班管用。你的声音、你的停顿、你的表情，都是孩子理解文字最好的脚手架',
        '听读先行：孩子的耳朵比眼睛走得快。一个7岁孩子可能只能自己读懂二年级的书，但他能听懂五年级的故事。朗读，就是在帮孩子的理解力"超前体验"',
        '朗读仪式：每天晚上八点，客厅的灯调暗一点，窝在沙发里，翻开书。不用很久，十五分钟就好。当这个时刻变成"我们家的习惯"，孩子会开始期待它',
        '从读给他听到他读给你听：小时候你读他听，慢慢地，他开始抢着读角色对话，再后来他自己捧着书不撒手了。这个过渡不用刻意，他准备好了，自然就发生了',
        '语感是读出来的：为什么有的孩子写作文语言很顺，有的磕磕绊绊？秘密就是"耳朵里存了多少好句子"。经常听好的语言，写的时候就有感觉了',
        '万物皆可读：路过一块牌子，读一读；超市里的商品包装，念一念；菜谱上的步骤，一起读出来。阅读不只是坐下来翻书这一种样子',
        '章节书朗读：绘本适合朗读，章节书也可以。每天读一章，在最悬念的地方停下来——"明天接着读"。这种"未完待续"的感觉，是培养阅读期待感最好的方式',
        '朗读不要停在孩子会自主阅读之后：很多家长觉得孩子能自己读了就不用朗读了。其实自己读的书和听大人读的书可以是不同层次的——朗读可以一直持续到小学高年级',
        '你的声音就是最好的有声书：手机里的故事音频再好听，也比不过你的声音。因为你会停顿、会看孩子的反应、会根据他的情绪调整节奏，这是任何音频做不到的',
        '读得不好也没关系：不会配音、不会演戏、普通话不标准？完全没关系。孩子在意的不是你读得有多专业，而是你愿意花时间陪他。真诚的陪伴，比完美的技巧有力量得多'
      ]
    },
    {
      source: '《书语者》（多纳琳·米勒）',
      focus: '每个孩子都可以成为"读书的人"，关键是让他自己相信这一点',
      methods: [
        '阅读身份：比起"读了哪本书"，更重要的是让孩子在心里觉得"我是一个爱读书的人"。这个身份一旦建立，他会自己去找书读。怎么建立？从"你读了多少"开始记录',
        '选书自由：让孩子自己逛书店、自己选书，哪怕他选了你觉得"太简单"的。自己选的书，读完的概率高三倍。强塞过来的经典，大概率在书包里吃灰',
        '轻量阅读记录：不用写读后感，不用摘抄好词好句。就一个本子，写上日期、书名、读到第几页、一句话感受。没压力，能坚持，积累的成就感是真实的',
        '40本书挑战：一年读40本书，听起来吓人？其实一周不到一本。关键是让孩子看到"我居然读了这么多"的惊喜感。量变引起质变，读的多了，品味自然就上来了',
        '弃书权：一本书读了三十页还是不想读？放下，换一本。大人追剧也会弃剧嘛。保护阅读的胃口，比啃完一本不喜欢的书重要一百倍',
        '阅读社区：孩子之间聊"你最近读了什么好书"比大人推荐管用多了。建一个小小的读书群、搞一次书籍交换会，让阅读变成一种社交货币',
        '榜样的力量：孩子需要看到身边有人在读书——不是被要求读，而是因为享受而读。当大人自己也在读，孩子不用被催促，自然会模仿',
        '扩展阅读类型：只爱读奇幻类？可以偶尔推荐一本现实主义的。不是强制替换，而是在他喜欢的基础上"旁边放一本"。口味是慢慢打开的',
        '给阅读正反馈：不要夸"你真棒，读了一本书"。可以说"你刚才提到的那个细节，我都没注意到"——夸具体的思考而不是行为，孩子会更有动力深入读下去',
        '阅读时间的保护：每天留出一段"不被打扰"的阅读时间，比每周末集中读两小时效果好。固定的、安静的时间段，是阅读习惯的底座'
      ]
    },
    {
      source: '《打造儿童阅读环境》（艾登·钱伯斯）',
      focus: '好的阅读不是教出来的，是"泡"出来的——环境、氛围、对话缺一不可',
      methods: [
        '阅读环境：沙发角落放个软垫，旁边摆一排书，再搭一盏暖灯。不用特别布置，关键是让书"随手可拿"。家里到处都能碰到书的孩子，阅读量是普通孩子的好几倍',
        '书话（Booktalk）：晚饭时大人随口说一句"我今天读到一本书，里面有个特别有意思的人……"，比说一百遍"去读书"有用。孩子需要看到大人也在读，而且读得津津有味',
        '阅读圈：三五个孩子选同一本书，各自读完后坐在一起聊。不是老师主持，不用回答问题，就是分享"我觉得最好玩/最讨厌/最没想到的部分"。读书这件事，一群人做比一个人有趣',
        '回应式讨论：别问"这本书的中心思想是什么"，换成"有没有哪个地方让你特别想说点什么？"。前者是考试，后者是聊天。孩子的思考在聊天里才能真正打开',
        '再读的价值：第一次读是读情节——"然后呢？然后呢？"第二次读才发现那些被忽略的细节——"原来这里就已经埋了伏笔！"好书值得反复读，每次都有新发现',
        '阅读的仪式感：每周五是"家庭读书夜"，每人选一本书，各读各的，中间分享一段。或者每月一次书店探险，预算内自己选书。当阅读和愉快的体验绑在一起，它就不再是任务了',
        '有吸引力的书架：把书的封面朝外摆放，定期更换"推荐位"的书。让书架看起来像一个小型书店橱窗，而不是一面整齐但无趣的墙',
        '大人的阅读姿态：孩子最敏锐的观察对象就是大人。你在沙发上读书还是刷手机？你谈论书还是谈论八卦？这些不经意的姿态，就是孩子对"阅读"的第一印象',
        '不急于推荐：有时候最好的策略是"不动声色"——把你觉得好的书放在孩子经常路过的地方，不说话、不推荐，等他自己拿起来。被"发现"的书比被"推荐"的书更有吸引力',
        '图书馆和书店的日常化：不是特地去"学习"的地方，而是像去公园一样自然的周末活动。逛书店、泡图书馆，让孩子觉得"去看书"和"去玩"是一样正常的事'
      ]
    },
    {
      source: '《阅读的力量》（斯蒂芬·克拉生）',
      focus: '大量的、自由的、快乐的阅读，本身就是最好的语文课',
      methods: [
        '自由自主阅读（FVR）：给孩子一段什么都不用做的时间——不考试、不写报告、不做笔记——就是安安静静读自己想读的书。研究证明，坚持FVR的孩子，阅读能力、写作能力、词汇量全面胜出',
        '兴趣驱动：孩子只爱看漫画？没关系。只爱看科普？也没关系。阅读的入口不重要，重要的是"他在读"。从漫画到小说，从科普到文学，兴趣会自己带路',
        '轻松阅读的力量：别小看那些"太简单"的书。大量读轻松的、能流畅阅读的书，是建立阅读自信和阅读速度的基础。就像学游泳，先在浅水区扑腾够了，才敢去深水区',
        '环境比说教重要：家里客厅有书架、卧室有书、卫生间都放着一本杂志——这个环境的力量，远远胜过"你要多读书"这句话。看得到书的孩子，才会拿起书',
        '不考试的阅读：读完一本书，第一句就问"学到了什么？写个读后感"——恭喜你，孩子对下一本书的兴趣又少了一分。阅读的快乐一旦和考试挂钩，就很难纯粹了',
        '阅读是习得的，不是教出来的：没有人是通过"学习阅读技巧"变成爱读书的人的。泡在书堆里，被好故事吸引着、裹挟着往前走，读着读着，能力就来了',
        '阅读量是基础：读得多不一定读得好，但读得少一定读不好。大量阅读是所有阅读能力的底层基础——词汇量、语感、理解力、写作能力，全都建立在"读得够多"上面',
        '尊重阅读口味：大人有自己的阅读偏好，孩子也有。不要因为你觉得"言情小说没营养"就阻止他。每一种阅读都在积累语言经验，口味会随着阅读量自然进化',
        '学校图书角的价值：教室里有没有一个图书角，对孩子的阅读量影响巨大。触手可及的书，远比楼下图书馆里的书更容易被翻开',
        '快乐是持续的前提：如果阅读是痛苦的，再好的方法也坚持不了。克拉生反复强调：任何让阅读变得不快乐的做法，都是在消耗未来的阅读力'
      ]
    },
    {
      source: '《如何阅读一本书》（莫提默·艾德勒）',
      focus: '阅读有层次，从"读完"到"读透"是不同的段位',
      methods: [
        '检视阅读：拿到一本新书，别急着翻第一页。先看封面、目录、序言，翻翻每章的开头和结尾。五分钟之内，你就能判断"这本书大概在说什么"。这个习惯能帮孩子省下很多读错书的时间',
        '分析阅读：读完之后，试着用一句话概括"这本书到底在讲什么"。如果说不出来，说明读是读了，但还没真正消化。能用自己的话说出来，才算读懂了',
        '四个基本问题：读任何一本书，都可以问这四个问题——这本书在说什么？是怎么说的？说得对吗？跟我有什么关系？教孩子带着这四个问题去读，阅读质量马上不一样',
        '主题阅读：孩子对恐龙着迷？太好了。找五本关于恐龙的书，对比着读——这本说恐龙灭绝是因为陨石，那本说是火山。不同观点碰在一起，批判思维就萌芽了',
        '分层理解：一本好书至少有三层意思——字面上说了什么，背后想表达什么，跟我的生活有什么关系。第一遍读到第一层就够了，等积累多了，再去探第二层、第三层',
        '做一个有要求的读者：不是所有书都值得同样认真地读。有些书翻翻就够了，有些书值得逐字逐句细读。教孩子分辨"这本书值得我花多少力气"，这本身就是一种重要的阅读能力',
        '用自己的话复述：读完一段，停下来用自己的话说一遍"刚才讲了什么"。说不出来？回去再看一遍。能说出来，说明真的理解了。这个简单的习惯，比任何阅读技巧都基础',
        '和作者"对话"：不要只是被动地接受。"作者说得对吗？有没有道理？我有没有不同的看法？"带着这种"对等"的心态去读，阅读就从"听课"变成了"讨论"',
        '先整体后局部：先快速浏览全书的结构和大意，再深入读每一章的细节。就像先看地图再出发，不会迷失在细节里。这种"先鸟瞰后聚焦"的能力，对学习任何科目都有用',
        '读书笔记不是摘抄：不是把书里的话抄下来，而是记录"我的反应"——这个地方让我惊讶、这个观点我不同意、这段话让我想到了另一件事。笔记的价值在于"你的思考"，而不是"书的内容"'
      ]
    },
    {
      source: '《整本书阅读》（课标实践）',
      focus: '从碎片阅读到完整阅读体验，培养深度阅读能力',
      methods: [
        '我的阅读档案：不是概括"这本书讲了什么"，而是记录三件事——"我最大的发现""我最大的困惑""我联想到了什么"。关注自己的感受比复述情节有意思多了',
        '人物小传：给书中最喜欢的角色写一份"简历"——名字、性格、经历过什么、最后变成了什么样的人。写完你会发现，这个角色比你以为的更复杂、更真实',
        '故事改写：如果让你当作者，你会怎么改结局？如果从反派的视角重讲这个故事，会是什么样的？这不只是写作练习，更是让孩子理解"每个故事都可以有不同的讲法"',
        '跨学科联读：读《海底两万里》可以连着海洋生物、潜水艇发展史一起聊；读《草房子》可以聊农村生活和童年。一本书就是一扇窗，窗外的风景远比故事本身大得多',
        '阅读时间线：拿一张长纸条，把故事里的事件按时间顺序排出来——先发生了什么，然后发生了什么，最后怎样了。故事的结构一下就看清了，孩子下次写作文，也知道怎么安排顺序了',
        '思维导图：用一张图把整本书"装"进去——主角在中间，分支是事件、人物关系、主题。画完之后退一步看看，整本书的脉络尽在眼前，而画的过程本身就是在深度思考',
        '给角色写信：挑一个书中的角色，写一封信给他。想对他说什么？有什么建议？有什么同感？这个练习让孩子从"旁观者"变成"参与者"，对故事的理解会深一个层次',
        '对比不同版本：同一个故事的不同版本（比如不同翻译、不同改编），拿来对比——哪个好？为什么？对比的过程就是在培养审美力和判断力',
        '读写联动：读完一本书，不一定要写读后感。可以试试续写一段、画一幅画、做一个手抄报、编一个小剧本。让"输出"变成自然的延伸，而不是被迫的作业',
        '班级共读：全班一起读同一本书，然后用不同的方式分享——有人画画、有人演戏、有人做PPT、有人辩论。当阅读变成集体体验，它的深度和乐趣都会翻倍'
      ]
    },
    {
      source: '《阅读力：文学作品的阅读策略》（阿德里安娜·吉尔）',
      focus: '教孩子像"文学侦探"一样读故事，读出文字背后的深意',
      methods: [
        '转变思维：好的读者不只是在"看故事发生了什么"，而是在想"作者为什么要这样写"。这个转变很关键——从被动跟着情节走，到主动思考作者的意图，阅读的深度完全不同',
        '推断能力：作者不会把所有事情都写出来。"他低下了头"——作者没说他难过，但你读出来了。教孩子发现文字"没说出口的话"，就是在培养推断能力，这是高级阅读的核心技能',
        '综合归纳：读完一段或一章，停下来想想"到目前为止，我对这个故事的理解是什么？"。随着阅读推进，理解会不断变化——这种"边读边更新理解"的能力，比读完再总结有用得多',
        '分析作者的写作手法：为什么作者要用"暴风雨"这个场景？为什么这段话特别短、节奏特别快？当孩子开始注意"作者是怎么做到的"，他就不只是读者了，也是学习者了',
        '与文本的深度对话：读到一个情节，不只是"然后呢"，而是"如果是我，我会怎么做？""我同意作者的安排吗？"把自己放进故事里，阅读就从旁观变成了参与',
        '主题探索：一本书的主题不会写在封面上，需要读者自己去"找"。同样一本书，不同的人读出不同的主题，都是对的。引导孩子去思考"这本书到底在说什么"，而不是给他一个标准答案',
        '情感线索追踪：注意人物的情绪变化——他什么时候开心、什么时候害怕、什么时候犹豫。情绪变化的地方往往是故事的转折点，也是理解人物的关键',
        '象征与隐喻：故事里的"暴风雨"可能不只是天气，"黑暗的森林"可能不只是一片树林。帮孩子慢慢建立"文字可能有言外之意"的意识，不用急，积累够了自然会有感觉',
        '从一本书到一类书：读完一本冒险小说，可以聊聊"冒险故事一般都有什么套路？"——英雄出发、遇到困难、获得帮助、最终成长。认识这些"故事模式"，读新书时会更敏锐',
        '尊重孩子的解读：孩子说"我觉得这本书在讲孤独"，你觉得在讲友情——两个都可以。文学没有标准答案，每个人的解读都值得被认真对待。鼓励他说出理由，比纠正他的答案重要'
      ]
    },
    {
      source: '《阅读力：信息类文本的阅读策略》（阿德里安娜·吉尔）',
      focus: '不只是读故事，科普、说明文、新闻也需要阅读策略',
      methods: [
        '信息类文本也需要策略：很多孩子读故事书没问题，一碰到科普书、说明文就头疼。因为读信息类文本需要不同的"脑回路"——不是跟着情节走，而是提取、组织、记住关键信息',
        '文本特征导航：科普书里的粗体字、小标题、图表、侧边栏，这些不是装饰，是"路标"。教孩子先看这些"路标"再读正文，就像看地图再上路，不容易迷路',
        '确定重要信息：一篇文章那么多内容，哪些是最重要的？帮孩子区分"作者觉得重要的"和"我觉得有趣的"，这两个经常不一样，而两个都值得关注',
        '质疑与求证：读到"蚂蚁可以举起自身体重50倍的东西"——真的吗？好的读者不会照单全收，会想想"这个信息可靠吗？我在别的地方也看到过吗？"。批判性阅读从信息类文本开始练最合适',
        '做笔记和整理：读科普书不能像读小说那样一口气读完。停下来，用自己的话概括一段话的意思；画个简单的表格对比信息——主动整理过的知识，才能真正记住',
        '从文本到生活：读了关于环保的文章，看看自家垃圾分类做得怎么样；读了关于天气的知识，明天抬头看看云是什么形状的。信息类阅读最好的归宿是"用起来"',
        '区分事实与观点：这句话是"事实"（可以被验证的）还是"观点"（作者的看法）？这个区分能力太重要了。很多孩子（甚至大人）分不清楚，而信息类文本是练习这个能力的最佳场地',
        '图表阅读能力：很多孩子看到柱状图、饼图、时间线就跳过。其实图表往往是文章最精华的部分——它用一张图就说清了文字需要一整段才能说的事。教孩子"先看图再看文"',
        '预习式阅读：在读正文之前，先扫一遍标题、小标题、图片说明、第一段和最后一段。花两分钟就能知道这篇文章大概在说什么，然后带着"框架"去读，效率和理解力都会大幅提升',
        '多源对比：同一个话题的两篇文章，观点可能完全不同。让孩子对比着读——"这篇说糖有害，那篇说适量没事，谁说得对？"这种多源对比的习惯，是信息时代最重要的阅读能力之一'
      ]
    },
    {
      source: '《说来听听：儿童、阅读与讨论》（艾登·钱伯斯）',
      focus: '阅读之后的讨论，才是理解真正发生的地方',
      methods: [
        '三种分享：钱伯斯说阅读讨论有三个层次——"我喜欢的""我不喜欢的""我困惑的"。不要上来就问"这本书讲了什么道理"，先从最简单的感受聊起，孩子的话匣子才能打开',
        '没有错误的回答：讨论不是考试，没有标准答案。"我觉得大灰狼也挺可怜的"——这个回答很好，因为孩子在思考。接住他的话，追问一句"为什么这样想？"就够了',
        '说来听听：当孩子说"我不喜欢这本书"，不要急着反驳。温柔地说一句"说来听听？"，然后认真听。这四个字比任何阅读技巧都管用——它传递的是"你的想法我在乎"',
        '从感受到思考：先聊"你读的时候什么感觉？"再聊"你觉得为什么会有这种感觉？"最后聊"你觉得作者想让你有这种感觉吗？"。三个问题，从表面感受一步步引向深度思考',
        '同伴讨论的力量：大人问孩子读后感，总有种"被检查"的感觉。但如果是几个孩子围在一起聊，氛围完全不同——"你也觉得那个地方很搞笑？""不是吧，我觉得很感人啊！"分歧和碰撞里，理解在加深',
        '讨论中的沉默：孩子不说话，不代表没在想。有时候一个好问题需要时间消化。给他留出安静思考的空间，比催他"说说你的想法"更有效',
        '追问的力量：第一个回答往往是表面的。真正的深度在第二、第三个追问里——"你说你觉得不公平，是哪个地方让你这样想的？""如果你是作者，你会怎么安排？"',
        '大人也要分享：不只是问孩子"你觉得呢"，你也说说自己的感受。"这一段让我很难过"或者"这个结局我也没想到"。当大人也真诚分享时，对话就从"考察"变成了"交流"',
        '允许分歧存在：两个孩子对同一本书的看法完全不同？太好了。分歧不是需要解决的问题，而是理解加深的机会。"你们说的都有道理，但注意到的地方不一样"——这句话就够了',
        '不是每本书都需要讨论：有些书读完就让它安静地待着就好。过度讨论和不讨论一样会消磨乐趣。观察孩子——他想聊的时候自然会开口，不想聊的时候别强迫'
      ]
    },
    {
      source: '《教出阅读力》（柯华葳）',
      focus: '阅读力是可以教的，关键是在对的时机用对的方法',
      methods: [
        '阅读理解的三个层次：字面理解（"故事说了什么"）、推论理解（"为什么会这样"）、评鉴理解（"我觉得怎么样"）。很多孩子卡在第一层，因为没人告诉他后面还有两层。提醒一下就好',
        '识字量不等于阅读力：认识所有的字，不代表读得懂。有的孩子认字很多但读完说不出个所以然——因为他在"解码"而不是在"理解"。阅读力的核心是理解，不是识字',
        '提问是金钥匙：读之前问"你觉得会说什么？"，读的时候问"到现在为止你最大的感觉是？"，读完问"如果你是主角会怎么做？"。三个时间点的三个问题，就把浅阅读变成了深阅读',
        '从"学习阅读"到"通过阅读学习"：低年级的目标是学会阅读，中高年级的目标是通过阅读去学其他东西。这个转折点很关键——如果中年级阅读力没跟上，后面所有科目都会受影响',
        '亲子共读不是读给他听就完了：共读的精髓在"共"——一起看图、一起猜测、一起讨论、一起翻页。不是大人读、孩子听，而是两个人一起参与这个故事',
        '差异化引导：每个孩子的阅读卡点不一样。有的是词汇不够，有的是背景知识缺乏，有的是注意力难集中。别用同一套方法对所有孩子，先观察他"卡在哪"，再针对性地帮',
        '背景知识的作用：同一本关于海洋的书，去过海边的孩子和没去过的孩子读出来的感觉完全不同。阅读理解力不只取决于技巧，还取决于孩子"知道多少"。多带他看世界，阅读力自然跟着长',
        '词汇量的自然积累：阅读力的底层是词汇量，但词汇不是靠背的，是靠在不同的语境中反复遇见。大量阅读是扩大词汇量最自然、最有效的方式——比任何词汇练习册都管用',
        '流畅度很重要：有的孩子一个字一个字地读，读完一句话已经忘了开头说什么了。这不是理解力差，是流畅度不够。解决方法很简单：多读简单的、能流畅阅读的书，速度上来了，理解力自然跟上',
        '阅读力需要日积月累：阅读力不是上一个培训班就能突飞猛进的，像弹钢琴一样需要持续练习。每天的阅读时间不需要很长，但需要"每天都有"。这种日积月累的力量，比任何速成方法都靠谱'
      ]
    }
  ];
  const randomSystem = readingMethodSystems[Math.floor(Math.random() * readingMethodSystems.length)];
  const randomMethod = randomSystem.methods[Math.floor(Math.random() * randomSystem.methods.length)];
  
  const toddlerTone = toddlerTones[Math.floor(Math.random() * toddlerTones.length)];
  const primaryTone = primaryTones[Math.floor(Math.random() * primaryTones.length)];
  
  const sections = {
    toddler: `
**## 幼儿段 (0-6岁)**
围绕"绘本阅读"和"亲子共读"，创作${count}段各具特色的文案。

**这次的语气是**：${toddlerTone.mood}  
${toddlerTone.hint}

**这次的方法论视角**：${randomToddlerSystem.source}  
核心理念：${randomToddlerSystem.focus}  
具体方法参考：${randomToddlerMethod}

**其他可用的方法（换换口味，别只围着一个方法转）**：
${randomToddlerSystem.methods.map(m => '- ' + m).join('\n')}

**写作心法（怎么把干货写得好读）**：
- **重感受**：不要只讲道理，要写出那个瞬间的氛围——灯光、体温、眼神、一句童言。让读者"看到"画面。
- **去焦虑**：涉及到误区或"不理想"的现象（如撕书、坐不住、只要同一本），语气要特别宽容，传递"这很正常"的安心感。
- **强实操**：给出的建议要小到马上就能做（比如"把书放低一格"、"读之前先看封面"），而不是"多陪孩子读书"这种正确的废话。
- **有节制**：每段只讲透一个点，不要贪多。三五句话说清一件事，比十句话说三件事有力量。
- **通用视角**：用"当孩子..."、"很多家庭会发现..."的普遍视角，严禁编造个人经历。

**排版**：一句一行，读起来有呼吸感  
**篇幅**：大约3-5句话，别太长
`,
    primary: `
**## 小学段**
围绕"阅读习惯与能力提升"，创作${count}段各有特点的文案。

**这次的语气是**：${primaryTone.mood}  
${primaryTone.hint}

**这次的方法论视角**：来自${randomSystem.source}  
核心理念：${randomSystem.focus}  
具体方法参考：${randomMethod}

**其他可用的方法（换换口味，别只围着一个方法转）**：
${randomSystem.methods.map(m => '- ' + m).join('\n')}

**写作心法（怎么把干货写得好读）**：
- **降维度**：把"批判性思维"这种大词，拆解成"多问几个为什么"这种小动作。让家长一看就知道怎么做。
- **给工具**：讲方法时，最好给一个具体的抓手——画一张图、玩一个游戏、问一个问题。抽象的建议不如一个可操作的动作。
- **懂心理**：理解这个阶段孩子对"被教育"的抗拒。站在孩子这边说话，比站在大人这边更有说服力。
- **温和纠偏**：涉及误区时，用"其实可以换个思路"而不是"你错了"。让读者觉得被理解，而不是被批评。
- **有节制**：每段只讲透一个点。四到六句话说清一件事，不贪多、不啰嗦。
- **通用视角**：用"很多孩子会..."、"你可能发现过..."的客观视角，严禁编造个人经历。

**排版**：一句一行，清爽利落  
**篇幅**：大约4-6句话
`
  };
  
  return sections[type];
}

// 生成幼儿段/小学段内容的提示词
function createSegmentPrompt(type: 'toddler' | 'primary', count: number) {
  // 更丰富的创意触发点，与新的书单体系深度匹配
  const toddlerAngles = [
    { theme: '从"图画书的特殊性"切入', example: '比如：文字没讲但图画讲了的故事、翻页带来的惊喜、环衬的秘密' },
    { theme: '从"无用之用"切入', example: '比如：读完没有任何教育意义、只是单纯好笑、孩子要求读第100遍' },
    { theme: '从"身体接触与安全感"切入', example: '比如：把孩子抱在怀里读、听心跳的声音、用手指着图画' },
    { theme: '从"听读的魔力"切入', example: '比如：声音的表情、重复的韵律、孩子闭着眼听' },
    { theme: '从"孩子的主导权"切入', example: '比如：让孩子自己翻页、自己选书、甚至自己编故事' },
    { theme: '从"阅读环境的细节"切入', example: '比如：沙发角的软垫、伸手可及的书架、大人读书的背影' },
    { theme: '从"共读中的对话"切入', example: '比如：孩子突然打断你、指着图问奇怪的问题、读完后的沉默' },
    { theme: '从"放下功利心"切入', example: '比如：不考识字、不问道理、只享受当下的快乐' },
    { theme: '从"选书的智慧"切入', example: '比如：为什么孩子爱读"烂书"、经典绘本好在哪里、不同风格的尝试' },
    { theme: '从"成长的瞬间"切入', example: '比如：第一次自己拿起书、第一次复述故事、第一次因书而笑' }
  ];
  
  const primaryAngles = [
    { theme: '从"阅读的权利"切入', example: '比如：有权不读完、有权读"闲书"、有权重读、有权跳读' },
    { theme: '从"像侦探一样思考"切入', example: '比如：寻找作者的伏笔、推测人物动机、发现文字背后的深意' },
    { theme: '从"聊书的艺术"切入', example: '比如：不问中心思想问感受、分享大人的困惑、允许孩子说"不喜欢"' },
    { theme: '从"非虚构阅读"切入', example: '比如：读科普书的策略、怎么看图表、怎么提取关键信息' },
    { theme: '从"阅读身份认同"切入', example: '比如：建立"我是读书人"的自信、记录阅读足迹、打造个人书单' },
    { theme: '从"朗读的延续"切入', example: '比如：大孩子也需要朗读、家庭朗读会、读出文字的节奏感' },
    { theme: '从"阅读与生活"切入', example: '比如：书里提到的地方想去看看、书里的美食想尝尝、书里的道理解决了现实问题' },
    { theme: '从"深读一本书"切入', example: '比如：画人物关系图、给角色写信、改写结局、跨学科联想' },
    { theme: '从"环境与社群"切入', example: '比如：家庭读书时间、和朋友聊书、逛书店的乐趣' },
    { theme: '从"克服阅读障碍"切入', example: '比如：读不下去怎么办、专注力不够怎么办、只爱看漫画怎么办' }
  ];
  
  const angle = type === 'toddler' 
    ? toddlerAngles[Math.floor(Math.random() * toddlerAngles.length)]
    : primaryAngles[Math.floor(Math.random() * primaryAngles.length)];

  // 使用共享的段落提示词
  const task_description = createSegmentSectionPrompt(type, count);

  return `直接返回纯JSON，不要任何额外说明或代码块标记。

# 角色定位：阅读陪伴者
你不是高高在上的教育专家，也不是虚构的"过来人家长"。
你是一位懂阅读、懂孩子、温暖而专业的陪伴者。你站在家长身边，用平等的视角分享阅读的美好与方法。

# 核心原则：真实与真诚
1. **绝对禁止虚构经历**：严禁使用"我家娃"、"我以前"、"我发现"、"我们家"等第一人称叙述虚构的生活场景。不要扮演一个不存在的家长。
2. **对话视角**：
   - 主要使用第二人称"你"（家长）和"孩子"。
   - 或者使用客观的第三人称视角描述普遍规律。
   - 示例："当孩子读不下去时，你可以试试..."（推荐） vs "我家孩子读不下去时，我用了..."（禁止）

# 语言风格指南
1. **专业但不晦涩**：把专业的阅读理论（如预测、联结）翻译成家长一听就懂的大白话。
2. **建议而非指令**：
   - 避免："你应该"、"必须"、"切记"。
   - 提倡："不妨试试"、"或许可以"、"你会发现"。
3. **温暖而有洞见**：语言要柔软，但要有力量。指出问题的同时，给出充满希望的解决方案。

# 这次可以试试
${angle.theme}  
${angle.example}

# 具体要求
${task_description}

# 注意
- ${count}段文案，每段都要有点不一样的味道
- 不要每段都用同样的句式开头
- 一句一行排版，读起来舒服就行

# 输出格式
{
  "copies": ["...", "...", "...", ...],
  "quote_index": -1
}

直接返回JSON，别加markdown标记。`;
}

// 名人名言生成提示词
function createQuotePrompt(quoteType: 'morning' | 'toddler' | 'primary', count: number) {
  const sceneConfig = {
    morning: {
      scene: '早安语场景',
      description: '适合早晨读到的名人名言，关于阅读、时光、成长、思考',
      themes: ['阅读与书', '时间与晨光', '成长与变化', '生活的智慧', '哲理与思考', '孤独与宁静'],
      vibe: '安静、温暖、有点诗意、有点哲思',
      avoidExamples: '书籍是人类进步的阶梯、读书破万卷下笔如有神、知识就是力量',
      goodSources: '作家（博尔赫斯、卡尔维诺、毛姆、伍尔夫）、哲学家（叔本华、蒙田）、诗人（辛波斯卡、里尔克、木心）、教育家（怀特海、杜威）'
    },
    toddler: {
      scene: '幼儿段场景',
      description: '适合低幼儿童家长的名言，关于陪伴、童年、想象力、天性',
      themes: ['童年的意义', '想象力与创造', '陪伴与爱', '天性与自由', '游戏与快乐', '慢成长'],
      vibe: '温柔、充满爱、让人想起孩子小时候的样子',
      avoidExamples: '孩子是上天赐予的礼物、每个孩子都是天使、陪伴是最长情的告白',
      goodSources: '儿童文学家（松居直、安徒生、林格伦）、心理学家（皮亚杰、温尼科特、蒙台梭利）、作家（纪伯伦、泰戈尔、圣埃克苏佩里）'
    },
    primary: {
      scene: '小学段场景',
      description: '适合小学生家长的名言，关于学习、独立思考、品格、求知',
      themes: ['独立思考', '阅读的深意', '坚持与毅力', '好奇与求知', '品格与勇气', '失败与成长'],
      vibe: '有力量但不鸡血、有哲理但不说教、让人想行动',
      avoidExamples: '少壮不努力老大徒伤悲、书山有路勤为径、天才是百分之一的灵感加百分之九十九的汗水',
      goodSources: '科学家（爱因斯坦、费曼、居里夫人）、作家（马克·吐温、海明威、鲁迅）、哲学家（苏格拉底、罗素）、教育家（陶行知、苏霍姆林斯基）'
    }
  };

  const config = sceneConfig[quoteType];

  return `直接返回纯JSON，不要任何额外文字或markdown标记。

# 角色
你是一位阅读领域的名言策展人，眼光独到，擅长从浩瀚的名人语录中挑出那些"读到第一遍就想收藏"的句子。

# 任务：精选${count}条${config.scene}的名人名言

**场景**：${config.description}

**可以的主题**：${config.themes.join('、')}

**想要的感觉**：${config.vibe}

# 选材标准（非常重要）

**真实性红线**：
- 所有名言必须是真实存在的、有据可查的。宁可少一条，也绝不编造。
- 如果不确定某句话是不是某人说的，就不要用。

**避免烂大街**：
- 明确不要这类：${config.avoidExamples}
- 不要"正确但无聊"的名言。要找的是读完让人"嗯，说得真好"的那种。

**推荐来源**：
- ${config.goodSources}
- 也可以找其他冷门但有深度的名人
- 中外不限，古今不限，但要确保引用准确

**多样性**：
- ${count}条名言来自${count}位不同的名人
- 涵盖不同时代、不同国家、不同身份（作家、哲学家、科学家、教育家等）
- 不要连续出现同一类型的名言

**格式**：
- 名言内容（一句一行排版，保持原文的节奏感）
- 换行后写：--作者名

# 输出格式
{
  "copies": [
    "名言内容\\n--作者名",
    "名言内容\\n--作者名",
    ...
  ],
  "quote_index": ${count - 1}
}

直接返回JSON，${count}条名言。`;
}

// 最美绘本语言生成提示词
function createPictureBookPrompt(category: 'minimalist' | 'childview' | 'philosophy' | 'nature', count: number) {
  const categoryConfig = {
    minimalist: {
      name: '极简主义',
      vibe: '简单的句子，舒缓的节奏，宁静、治愈，有点淡淡的孤独感或禅意。没有华丽的修辞，每个字都恰到好处',
      like: '像《山中旧事》《大海遇见天空》《安的种子》《团圆》那种',
      avoid: '不要选情节复杂的段落，不要选对话密集的部分。要的是那种"安静得能听到呼吸"的文字',
      examples: [
        '小时候，我住在山里。\n我从没向往过海洋，\n也从没向往过沙漠。\n因为我住在山里，就已经足够。\n——《山中旧事》'
      ]
    },
    childview: {
      name: '儿童视角',
      vibe: '从孩子眼睛看世界，切入点非常小（看猫头鹰、游泳、住在山里），但指向的是人生的大命题：孤独、满足、自信、追寻',
      like: '像《月下看猫头鹰》《石头汤》《田鼠阿佛》《小房子》那种',
      avoid: '不要选大人视角写的"假儿童"文字。要的是真正透过孩子的眼睛看到的世界',
      examples: [
        '出去看猫头鹰，\n不需要说话，\n不需要温暖舒适，\n也不需要别的什么，\n只要心中有一个希望。\n——《月下看猫头鹰》'
      ]
    },
    philosophy: {
      name: '哲理留白',
      vibe: '文字有空气感，有留白，读完让人想静静体会一下。通感与留白并存，文字在捕捉一种"说不清但感受得到"的东西',
      like: '像《大鲸鱼玛丽莲》《一片叶子落下来》《活了100万次的猫》《失落的一角》那种',
      avoid: '不要选说教性强的段落。要的是那种"不给答案，但让你自己想到了答案"的文字',
      examples: [
        '"你觉得自己怎么样，你就会怎么样。\n觉得自己轻，就能学好游泳。\n想想看，鸟和鱼会觉得自己重吗？当然不会！"\n想变轻，就要觉得自己轻。\n试一试吧！\n——《大鲸鱼玛丽莲》'
      ]
    },
    nature: {
      name: '自然隐喻',
      vibe: '通过描写自然（风、山、光影、植物、水）来隐喻人的内心世界或人生真理。自然不只是背景，而是主角',
      like: '像《宝藏》《森林大熊》《花婆婆》《小种子》《树真好》那种',
      avoid: '不要选自然只是"布景"的段落。要的是"自然本身在说话"的文字',
      examples: [
        '"有时候，\n人必须远行，\n才能发现近在咫尺的东西。"\n——《宝藏》'
      ]
    }
  };

  const config = categoryConfig[category];

  return `直接返回纯JSON，不要任何额外文字或markdown标记。

# 角色
你是一位绘本文字的"品鉴师"，对绘本语言有极高的审美标准。你要找的不是"还不错"的文字，而是"读完会起鸡皮疙瘩"的文字。

# 任务：从真实绘本中摘录${config.name}风格的文字

**风格定义**：${config.vibe}

**参考书**：${config.like}

**示例**：
${config.examples.join('\n\n')}

# 选材标准（非常重要）

**真实性红线**：
- 必须是真实绘本的原文，一个字都不能改
- 保留原书的断行节奏（用\\n表示换行）
- 格式：原文内容 + 换行 + ——《书名》
- 如果不确定是不是原文，就不要用

**审美标准**：
- ${config.avoid}
- 每一条都要让人读完有"停下来想一想"的冲动
- 宁缺毋滥：找不到足够好的，宁可少一条也不凑数

**选书策略**：
- ${count}条必须来自${count}本不同的绘本
- 明确回避：《猜猜我有多爱你》《爱心树》《逃家小兔》《小王子》——这些被引用太多了
- 优先找：凯迪克奖、格林威奖、博洛尼亚奖获奖作品中的冷门佳作
- 来源多元：欧美、日本、中国原创绘本都可以
- 经典和近年新作都行，但质量第一

**长度**：
- 随原文而定，可长可短
- 短的可以只有两三行，长的可以是完整的一段
- 优先保证内容的完整性和节奏感

# 输出格式
{
  "copies": [
    "原文内容\\n——《书名》",
    "原文内容\\n——《书名》"
  ],
  "quote_index": -1
}

直接返回JSON，摘录${count}条。`;
}

// 生成早安语内容的提示词
function createMorningPrompt(
  dateString: string, 
  context: any,
  regenerate_column?: 'morning' | 'toddler' | 'primary'
) {
  const displayDate = context.formattedDate || dateString;

  // 早安语认知素材库：6个维度，每个维度8条具体素材
  const morningThemes = [
    {
      dimension: '阅读的意义',
      description: '为什么读书？读书到底改变了什么？',
      seeds: [
        '读书不是为了记住什么，而是为了在某个未来的瞬间，突然理解了一件原本不理解的事',
        '一个读过故事的孩子，比没读过的孩子多了一种理解世界的方式——他知道"别人可能和我想的不一样"',
        '阅读最大的作用不是增长知识，而是扩展"可能性"——让孩子知道世界不只有眼前这一种样子',
        '书不会直接告诉孩子怎么活，但它会在孩子心里存下很多"种子"，遇到合适的土壤就会发芽',
        '为什么要读书？不是为了考试，不是为了变聪明，而是为了在漫长的人生里，永远有一个可以安放自己的地方',
        '读过书的人和没读过的人，区别不在于知道多少，而在于面对未知时，是恐惧还是好奇',
        '阅读是一种"慢变量"——它不会让孩子明天就变好，但十年后回头看，读过书的人生和没读过的，轨迹完全不同',
        'C.S.刘易斯说过：我们读书，是为了知道自己并不孤独。每一本书都在说：你的感受，有人懂'
      ]
    },
    {
      dimension: '教育观与心态',
      description: '什么是好的教育？如何对抗焦虑？',
      seeds: [
        '教育最难的部分不是"教"，而是"等"——等孩子按自己的节奏长大，等种子在看不见的地方生根',
        '焦虑的反面不是放松，是信任——信任孩子有自己的时区，信任你做的每一点小事都在生效',
        '好的教育不是把桶装满，而是把火点燃。你不需要给孩子所有答案，只需要保护他提问的勇气',
        '我们总想让孩子"赢在起跑线"，但人生不是短跑。那些慢慢来的孩子，往往走得更远、更稳',
        '教育的本质是一棵树摇动另一棵树。你不需要做一棵完美的树，只需要在风中和孩子一起摇摆',
        '不是所有的"落后"都需要追赶。有些孩子只是在用不同的方式前进，而你需要做的只是不打扰',
        '蒙台梭利说：每个孩子心里都有一张成长的蓝图，大人要做的不是画图，而是提供阳光和土壤',
        '比起"你今天学了什么"，更重要的问题是"你今天好奇了什么"。好奇心是一切学习的真正起点'
      ]
    },
    {
      dimension: '童年与理解孩子',
      description: '童年为什么重要？怎么真正理解孩子？',
      seeds: [
        '童年不是成年的准备期，它本身就是完整的。孩子不是"未来的大人"，他现在就是一个完整的人',
        '孩子看世界的方式和大人完全不同。一片落叶、一只蚂蚁、一朵云，在他眼里都值得停下来认真看看',
        '《窗边的小豆豆》里，小林校长对小豆豆说的第一句话是"你跟老师说说话吧"——然后他听了四个小时。有时候，教育就是这么简单：认真听',
        '孩子的"为什么"不是在找答案，而是在表达好奇。回答不了也没关系，"我们一起去找找看"比正确答案更有价值',
        '我们总在教孩子如何成功，却很少教他们如何面对失败。而童年恰恰是学习"失败也没关系"的最佳时机',
        '皮亚杰说：孩子不是小号的大人，他们有自己的逻辑和智慧。很多时候，是大人需要向孩子学习——学习如何好奇、如何专注、如何快乐',
        '童年最珍贵的不是学到了什么，而是被允许做自己。一个被充分接纳的孩子，长大后才能真正接纳世界',
        '孩子的快乐很简单——一本喜欢的书、一个愿意陪他的人、一段不被打断的时光。我们经常把简单的事情想复杂了'
      ]
    },
    {
      dimension: '家长自我成长',
      description: '家长自己的阅读和成长同样重要',
      seeds: [
        '你不需要读完所有育儿书才能做好父母。有时候，读一本和育儿无关的书，反而让你成为更好的自己——而更好的自己，就是更好的父母',
        '孩子不会听你说了什么，但会看你做了什么。你在沙发上翻书的样子，比任何说教都有力量',
        '疲惫的时候，允许自己什么也不做。阅读教会我们最重要的一件事是：空白也是故事的一部分',
        '你为自己读的每一本书，都在间接影响着孩子。因为你的认知边界，就是他最初的世界边界',
        '完美的父母不存在，但"愿意成长的父母"就已经足够好了。你在读这段文字，说明你正在成长',
        '别总觉得陪孩子就是"牺牲自己的时间"。当你放下手机、捧起一本书，你既在陪伴孩子，也在陪伴自己',
        '成年人的阅读和孩子不同——我们不只是在读故事，也在故事里找自己。有时候一本童书，反而比成人书更能触动你',
        '如果你今天很累，不想做"好家长"——那就只做一个真实的人就好。真实比完美更有力量，孩子能感受到'
      ]
    },
    {
      dimension: '慢与等待',
      description: '教育中的耐心、长期主义、不功利',
      seeds: [
        '种一棵树最好的时间是十年前，其次是现在。阅读也一样——什么时候开始都不晚，但今天就是最好的时候',
        '孩子的成长不是匀速的，它更像竹子——地下扎根三年看不到动静，一旦破土就势不可挡',
        '我们总想看到"效果"——读了多少本、认了多少字、考了多少分。但阅读最深远的影响，恰恰是那些无法被量化的东西',
        '慢一点没关系。在一个什么都求快的时代，愿意陪孩子慢慢读一本书，本身就是一种勇气',
        '不是所有的"有用"都能立刻看到。今天读的那本书，可能要到十年后才会在某个意想不到的瞬间发挥作用',
        '耐心不是什么都不做地等着。耐心是每天做一点小事——读一页书、讲一个故事——然后相信这些小事会累积成大事',
        '日本有句话叫"一期一会"，意思是此刻的相遇不会再来。今天和孩子一起读的这本书、此刻的灯光和笑声，都是不可复制的',
        '龙应台说：所谓父母子女一场，不过是目送他的背影渐行渐远。趁他还愿意窝在你怀里听故事的时候，多读几本吧'
      ]
    },
    {
      dimension: '阅读与生活',
      description: '阅读如何改变我们看世界的方式',
      seeds: [
        '读过《小王子》的人，看到玫瑰花的时候会多想一秒。阅读不会改变世界，但会改变你看世界的眼睛',
        '一个常读书的人和不读书的人，走在同一条街上看到的东西是不一样的。阅读给了我们更多"看见"的能力',
        '书是唯一的时光机——你可以去古希腊和苏格拉底对话，也可以去未来看看人类会变成什么样',
        '阅读最神奇的地方在于：你明明坐在原地没动，心却走了很远很远的路',
        '生活给不了你所有的体验，但书可以。你可以在书里经历一千种人生，然后更理解自己这一种',
        '有时候读到一句话，会突然觉得"原来有人和我想的一样"——这种被理解的感觉，是阅读给人最大的温暖',
        '一本好书读完之后的那个下午，世界好像变得更安静了一点、更清晰了一点。这就是阅读给生活加的"滤镜"',
        '读书和旅行很像——都是离开原地，去一个新的世界走一趟，然后带着不一样的自己回来'
      ]
    }
  ];
  const randomTheme = morningThemes[Math.floor(Math.random() * morningThemes.length)];
  const randomSeed = randomTheme.seeds[Math.floor(Math.random() * randomTheme.seeds.length)];

  // 写作角度（与素材库配合）
  const writingAngles = [
    { angle: '从一个具体的认知出发', hint: '围绕素材库中的那个观点展开，说透它' },
    { angle: '从一个反问切入', hint: '先抛一个问题，再用素材中的观点回应' },
    { angle: '从一个画面展开', hint: '先描述一个安静的场景，然后自然引出思考' },
    { angle: '从对比入手', hint: '对比两种状态——读与不读、快与慢、焦虑与平静' },
    { angle: '从一句引用生发', hint: '围绕某位大家的话，展开自己的理解' },
    { angle: '用一个比喻串起来', hint: '把阅读/教育/成长比作某个日常的事物' }
  ];
  const angle = writingAngles[Math.floor(Math.random() * writingAngles.length)];

  // 季节意象库：把每个季节拆成可感知的细节，而不是一句抽象的话
  const seasonImagery: Record<string, { feel: string; details: string; avoid: string }> = {
    '春季': {
      feel: '万物松动，空气里带点潮气，心里也跟着软下来',
      details: '柳芽冒头、雨后的石板路、窗外忽然多出来的鸟叫、白天一点点变长、午后的阳光开始温吞',
      avoid: '别写落叶、厚雪、丰收、萧瑟、干燥'
    },
    '夏季': {
      feel: '时间被拉得很长，光线很重，人容易恍惚',
      details: '知了声、竹席的凉意、暴雨前的闷、晚风里的洒水声、傍晚七点天还亮着、西瓜皮在冰箱里',
      avoid: '别写落叶、枯枝、初春的嫩芽、寒风'
    },
    '秋季': {
      feel: '温度在降，人也跟着安静下来，想做点安静的事',
      details: '银杏、桂花香、早晚的温差、天色变高、下午的阳光金黄、风里第一次带凉意',
      avoid: '别写蝉鸣、新芽、炎热、雪'
    },
    '冬季': {
      feel: '世界被拧小了一圈，人更愿意待在"里面"',
      details: '呵气成白、窗外的枯枝、四五点就开始暗、暖气和毛毯、一杯热的东西、脚一直是凉的',
      avoid: '别写花开、绿意、蝉鸣、炎热'
    }
  };
  const seasonImg = seasonImagery[context.season];

  // 节气钩子库：全 24 节气，每个节气配 5 条可用灵感种子
  const solarTermHooks: Record<string, { imagery: string; seeds: string[] }> = {
    '小寒': {
      imagery: '一年里最冷的门槛，窗上结霜、手脚发僵、天黑得早',
      seeds: [
        '"最冷"与"最静"的同构——外面越冷，里面越适合一个人安安静静',
        '寒冷会把人收回身体里，反而让阅读变得专注',
        '冬天的早晨不适合仪式感，适合慢慢醒来、慢慢打开一本书',
        '小寒教给我们的事——不是所有"冷"都要抗争，有些就适合被它裹着',
        '最冷的日子里，一盏灯、一本书、一杯热的东西，就是全部的世界'
      ]
    },
    '大寒': {
      imagery: '冷到极处，但已经能闻到一丝松动的气息',
      seeds: [
        '"极"的另一面就是转——大寒之后，春天就开始倒数了',
        '大寒像一场漫长的屏息，屏到底之后自然会呼出来',
        '冬天攒下的那些耐心，都会在春天里开花',
        '至暗之前，先把自己照顾好——一杯热茶、一本不用动脑的书',
        '大寒是一年写给自己的收尾——还有什么没想通的，这几天想一想'
      ]
    },
    '立春': {
      imagery: '名义上的春天，其实还冷，但心里的开关已经拨了一下',
      seeds: [
        '"开始"不是热闹，是一点点松动——冰还没化，但已经听见水声',
        '立春适合做一件小事：翻开一本之前没读完的书',
        '每一个春天都在假装自己是第一次——这种天真其实很珍贵',
        '节气的智慧是"不急"——春天说自己来了，但它不会一下子到',
        '立春给孩子讲的故事，可以关于等待、关于苏醒、关于慢慢来'
      ]
    },
    '雨水': {
      imagery: '冰雪化成水，空气发潮，第一声春雷还没来',
      seeds: [
        '雨水教给我们的是：硬的东西要慢慢化，快不来',
        '下雨天适合读书——外面越模糊，里面越清晰',
        '给孩子读雨的故事：雨在唱歌、雨在敲窗、雨在把世界擦亮',
        '"润物细无声"——最深的影响，都是慢慢渗进去的',
        '雨水之后，空气里有股土的味道——那是一整个春天的预告'
      ]
    },
    '惊蛰': {
      imagery: '春雷惊醒冬眠，土地开始有动静',
      seeds: [
        '惊蛰像一个响指——什么东西被唤醒了，哪怕还不知道是什么',
        '有些书就是惊蛰——读完之后，心里有个沉睡很久的东西动了一下',
        '对孩子来说，每天都可能是"惊蛰"——一个词、一幅图、一只虫，就能打开一整个世界',
        '沉睡多久的东西，一声响就能活过来——这也是阅读对人做的事',
        '春天是从地下开始的——看得见的热闹之前，总有看不见的动静'
      ]
    },
    '春分': {
      imagery: '昼夜平分，阳光变得诚恳，花开始集中出现',
      seeds: [
        '春分的智慧是"平衡"——白天和黑夜说好了各占一半，不争',
        '"平分"是一种温柔的秩序——每件事都有自己的时间',
        '春天走到一半的时候，适合停下来问问：今年想读点什么',
        '阳光变得很诚恳——照到哪里就是哪里，不含糊',
        '春分教给孩子的事：公平不是"都一样"，而是"各得其所"'
      ]
    },
    '清明': {
      imagery: '天气清朗，也是追思的日子，雨容易落下来',
      seeds: [
        '清明是一年里难得的"向后看"——过去的人通过记忆回来一次',
        '让逝去的人说话的方式，就是重讲他们讲过的故事',
        '孩子第一次听到"死亡"，可能就是在清明——而一本好书，能帮他慢慢理解',
        '清明的雨，像是时间自己在哭一次——然后继续往前走',
        '我们读书，有一部分就是为了和那些再也见不到的人还能"说话"'
      ]
    },
    '谷雨': {
      imagery: '雨生百谷，春天最后一个节气，万物已经站稳',
      seeds: [
        '谷雨是春天的收尾，也是夏天的排练',
        '"雨生百谷"——重要的事情都需要水和时间',
        '读书像下雨——你不知道具体哪一滴在起作用，但每一滴都在',
        '春天快结束了，那些春天里开的花，该去看一眼了',
        '谷雨之后，世界会一下子变热闹——在那之前，安静几天'
      ]
    },
    '立夏': {
      imagery: '暑气上场，白天开始长得惊人',
      seeds: [
        '立夏不是"夏天到了"，是"夏天来了请注意"',
        '白天变长——多出来的那一点光，可以用来读几页书',
        '夏天对孩子来说，是一年里最饱满的部分——让他多在外面跑，也多在书里住',
        '盛夏之前，先学会安顿——不被热浪推着走',
        '立夏读书的心法：放慢一拍，别急着跟夏天的节奏'
      ]
    },
    '小满': {
      imagery: '麦粒开始饱满，但还没熟——一切刚刚好，不到顶峰',
      seeds: [
        '"将满未满"是中国人最懂的美学——差一点点就是最好',
        '小满教我们的是：不追求圆满，也不要求自己全懂',
        '读书也是一个小满的状态——知道一点，但还在路上',
        '给孩子的教育，最忌"小满之后继续催熟"——让他自己走到大满',
        '"满招损，谦受益"——小满是提醒，不是终点'
      ]
    },
    '芒种': {
      imagery: '有芒的麦子可收，有芒的稻子可种，最忙的农时',
      seeds: [
        '芒种是一年里最忙的节气，但古人还留出时间过节——忙不是借口',
        '"忙"与"乱"之间差一本好书的距离',
        '越忙越要读书——不是为了补知识，是为了在忙里保留"自己"',
        '给孩子看：大人最忙的时候，还是会捧起一本书——这就是榜样',
        '芒种的智慧：收成的同时就在播种——阅读也是，边读边种'
      ]
    },
    '夏至': {
      imagery: '一年里白天最长的一天，从此白昼开始变短',
      seeds: [
        '夏至最动人的一面是"从最亮开始变暗"——顶点即转折',
        '白天最长的这一天，可以安安静静多读几页书',
        '盛大里要有冷静——否则盛大就只是吵闹',
        '夏至提醒我们：不是所有"越来越多"都要庆祝，也有"够了"的智慧',
        '给孩子讲夏至：今天之后，日头就会一天比一天短——他第一次理解"循环"'
      ]
    },
    '小暑': {
      imagery: '热度上升但还没到极点，蝉开始叫',
      seeds: [
        '小暑是热的序章——还没到最热，但心里要有数',
        '蝉声起来的时候，适合读有夏天气味的书',
        '热会让人浮躁，而阅读恰好是一剂凉的',
        '小暑的傍晚，是给孩子讲故事最好的时候——风还没凉，但声音已经安静',
        '越是热闹的季节，越要给自己留一个安静的角落'
      ]
    },
    '大暑': {
      imagery: '一年里最热的时候，时间黏稠',
      seeds: [
        '大暑的时间是黏稠的——读书反而慢得下来',
        '最热的几天，什么也不做才是正经事——除了读书',
        '高温里读一本厚书，像把自己泡在一汪凉水里',
        '大暑提醒我们："极致"是一种状态，但不是一种常态',
        '给孩子的夏天，不止是西瓜和水枪，还可以是一本读不完的长故事'
      ]
    },
    '立秋': {
      imagery: '名义上的秋，其实还热，但风里已有一丝不同',
      seeds: [
        '立秋是一年里最细微的转折——敏感的人先感觉到',
        '不是所有"转折"都有响声——有时候只是风里的一丝凉',
        '季节的翻面往往先在书里发生——突然想读不一样的书了',
        '立秋不是"秋天到了"，是"秋天准备来了"——给自己一点时间转向',
        '从盛大转入收敛——立秋是全年最重要的节奏调整'
      ]
    },
    '处暑': {
      imagery: '暑气收尾，早晚开始凉下来',
      seeds: [
        '处暑是暑气的"退场"——它走得很慢，但确实在走',
        '喧闹之后的余韵，往往比喧闹本身更长',
        '处暑的傍晚开始凉，适合关上空调打开窗户读书',
        '所有的"过去"都值得被正式地告别——夏天也是',
        '处暑给孩子的一课：热闹会结束，但会留下一点东西——比如一个夏天的故事'
      ]
    },
    '白露': {
      imagery: '夜里开始有露水，空气里有水汽的光',
      seeds: [
        '白露是最诗意的节气——"蒹葭苍苍，白露为霜"',
        '秋意是慢慢显现的——白露这天，大概是第一次真正感觉到',
        '读书也是一种"显现"——读的时候不知道，多年后才显出它的样子',
        '早起的人先看见露水——敏感和早起，常常是一体的',
        '白露的晨光，有一种被洗过的干净——适合读干净的书'
      ]
    },
    '秋分': {
      imagery: '昼夜再次平分，从此夜长于昼',
      seeds: [
        '秋分之后，"向内"的季节开始了——夜长，心也长',
        '平衡不是持续的，只是某一瞬间的——秋分这一天就是',
        '白天开始短了——意味着晚上开始长，读书的时间多了',
        '秋天是适合读厚书的季节——时间稳了，心也稳了',
        '秋分教孩子的事：长短会变——重要的是适应，不是抱怨'
      ]
    },
    '寒露': {
      imagery: '露水带寒意，秋意深了一层',
      seeds: [
        '寒露之后，"凉"变成了"冷"——小心地加一件衣服',
        '秋天已经深了——适合读一点有分量的书',
        '这个时节的光，是"斜的"——不正、不热、但很耐看',
        '寒露提醒我们：秋天不只是收获，还有一丝清醒',
        '给孩子的寒露：风里有不一样的味道了——让他学会闻季节'
      ]
    },
    '霜降': {
      imagery: '草木披霜，秋的最后一个节气',
      seeds: [
        '霜降是秋天的告别词——不煽情，但郑重',
        '被霜打过的植物，有一种特别的质感——人也是',
        '一年快要"收"了——适合回头看看读过哪些书',
        '秋天的尾声总有一丝辽阔——风把一切都吹得很远',
        '霜降像一个句号——把这一年大部分的故事都收进去'
      ]
    },
    '立冬': {
      imagery: '正式入冬，开始"藏"的季节',
      seeds: [
        '立冬的主题是"藏"——该收起来的收起来，该留下来的留下来',
        '冬天不是什么都不做的季节——是把看不见的事做好的季节',
        '一年里真正属于"读书"的季节开始了',
        '立冬给孩子讲"藏"——不是"没有"，是"暂时不见，明年再出来"',
        '立冬之后，一日三餐开始变得有仪式感——读书也该有'
      ]
    },
    '小雪': {
      imagery: '北方开始飘雪，南方阴冷加深',
      seeds: [
        '小雪是冬天的"序曲"——雪不大，但天地的调子变了',
        '"雪"是一种让人安静的声音——下的时候读书，特别有氛围',
        '南方没有雪，但有阴冷——同样适合一盏灯、一本书',
        '给孩子的小雪：第一朵雪花落下来的时候，让他停下来看一看',
        '冬天的好，是它会逼你慢下来——而这恰恰是读书最需要的'
      ]
    },
    '大雪': {
      imagery: '雪开始成形，世界被覆盖得安静',
      seeds: [
        '大雪像一张可以重写的纸——一年里难得的"空白"',
        '白和静是同义词——雪让世界安静',
        '大雪之后，走路都变得有声音——咯吱咯吱，像在翻一本很厚的书',
        '冬天深处读的那本书，往往会记很久',
        '给孩子的大雪：让他在雪里踩一脚，然后回家读关于雪的故事'
      ]
    },
    '冬至': {
      imagery: '一年里夜最长的一天，从此白昼回升',
      seeds: [
        '"冬至一阳生"——最深处往往是转机',
        '冬至像一个隐秘的生日——天黑到不能再黑，光开始回来',
        '夜最长的这一天，读什么书会记一辈子',
        '冬至这一天，适合想想一年里哪些事其实已经开始松动',
        '给孩子的冬至：过了今天，白天就一天天长回来——这是一种最朴素的希望'
      ]
    }
  };

  // 节日钩子库：每个节日配 5 条灵感种子 + 意象 + 避坑（共 23 个节日）
  const festivalHooks: Record<string, { imagery: string; seeds: string[]; avoid: string }> = {
    '元旦': {
      imagery: '新一年的第一天，一切按下开始键但其实没什么不同',
      seeds: [
        '"新开始"的真相——不是万象更新，而是愿意再来一遍',
        '元旦适合列一份简单的年度阅读愿望清单——一本就够',
        '新一年最好的仪式，是翻开一本今年要读的书',
        '去年没读完的书，可以今天捡起来——不丢脸，挺动人',
        '元旦是一个温柔的起点——允许自己从"一般般"开始'
      ],
      avoid: '别堆"新年新气象"的套话；别写成朋友圈文案'
    },
    '春节': {
      imagery: '团聚、年夜饭、红色、鞭炮、孩子守岁、一家人难得都在',
      seeds: [
        '过年给孩子读的故事，可能是他一生记住的故事',
        '家的含义不是房子，是那些一起听过同一个故事的人',
        '长辈讲的那些旧事，本身就是一种阅读',
        '春节最动人的不是热闹，是热闹之后深夜里的那一点安静',
        '过年最珍贵的阅读时光——是所有人都在家的那几天'
      ],
      avoid: '别写"恭喜发财"的客套；别假装我在包饺子'
    },
    '元宵节': {
      imagery: '月圆、花灯、汤圆、年的尾声',
      seeds: [
        '元宵节是年的"最后一页"——这一天过了，生活正式恢复',
        '"圆"不是完美，是一家人能凑到一起',
        '花灯是最古老的儿童读物——图、字、光，一次到位',
        '元宵节适合给孩子讲一个关于月亮的故事——中国人的月亮和西方不一样',
        '年的故事会结束，但故事的习惯可以留下来'
      ],
      avoid: '别写得像广告词'
    },
    '情人节': {
      imagery: '被商业包装过但依然有人认真过的日子',
      seeds: [
        '爱是一件需要练习的事——书里那些最动人的爱，都是练出来的',
        '父母对孩子的爱，也是一种情人节——只是没有玫瑰',
        '最好的情话可能来自书里，而不是微信里',
        '情人节读一本和爱有关的书——不止男女之爱，还有友情、亲情、对世界的爱',
        '被理解，是爱最深的那一层——而书常常是第一个理解你的人'
      ],
      avoid: '别迎合过度浪漫；别只写男女之爱'
    },
    '妇女节': {
      imagery: '属于女性的一天，背后有长长的争取史',
      seeds: [
        '每一个妈妈，本来都是一个女孩',
        '女性阅读者这个身份，其实不到两百年的历史——今天我们能读书，是几代人争来的',
        '那些塑造了我们的女作家：伍尔夫、奥斯汀、门罗、杨绛……',
        '母亲的自我，也是孩子第一本看的书',
        '"做自己"不是口号——是每一个被母亲身份淹没过的女人都懂的事'
      ],
      avoid: '别写"女神节"的营销话术'
    },
    '植树节': {
      imagery: '春天刚刚开始，种下一棵树，种下一个未来',
      seeds: [
        '种树最好的时间是十年前，其次是今天——读书也是',
        '孩子不是一棵被规划好的树——他自己会长出自己的形状',
        '教育像种树——你只能浇水、等天气，不能拽着它长',
        '一本书像一棵树——种下去时看不出什么，十年后才发现它挡住了一场风',
        '植树节给孩子读一本关于树的绘本——让他理解"时间"'
      ],
      avoid: '别喊环保口号；别写得像作文'
    },
    '愚人节': {
      imagery: '允许说假话的一天，其实也是认真看待"真"的日子',
      seeds: [
        '愚人节最好的玩法，是一本书里的一个反转',
        '童书里有很多精彩的"愚人"——有时候傻气才是最聪明的',
        '允许孩子开一个无伤大雅的玩笑——幽默感是从这里开始的',
        '认真的反面不是玩笑——是麻木',
        '愚人节读一本带点荒诞的书——童书里的荒诞是最高级的'
      ],
      avoid: '别写"警惕谎言"的说教'
    },
    '世界读书日': {
      imagery: '每年一次，所有爱书的人在同一天抬头看向彼此',
      seeds: [
        '世界读书日不是一个"要读书"的日子，是一个"还在读书"的庆祝',
        '今天可以做的一件小事——给孩子读一段你最喜欢的话',
        '我们为什么需要一个"读书日"？因为读书越来越不是一件自然的事',
        '世界读书日最好的纪念方式，是今晚少看一小时手机',
        '一起读过书的人之间，有一种不必解释的默契——今天适合想起他们'
      ],
      avoid: '别写成图书馆广告'
    },
    '劳动节': {
      imagery: '放假、远行、人群、春末夏初',
      seeds: [
        '"劳动"不只是工作——读书、思考、带孩子，都是劳动',
        '慢下来也是一种劳动——而且是最难的那一种',
        '旅途中的阅读自有一种魔力——风景和文字互相翻译',
        '劳动节给孩子讲一讲你是做什么的——比你想象的有意义',
        '假期的第一天最适合读书——人还没野，心刚刚松'
      ],
      avoid: '别说教"劳动最光荣"'
    },
    '青年节': {
      imagery: '年轻的日子，属于提问与探索',
      seeds: [
        '"年轻"不是一段时间，是一种状态——保持好奇就还年轻',
        '青年最好的武器是一本书——因为它不告诉你答案，逼你自己想',
        '每一个大人，心里都有一个未完成的青年——那个人读过什么书，影响着现在的你',
        '青年节给自己读一本当年错过的书——补一个人生的课',
        '这一代青年读的书，会变成下一代孩子的底色'
      ],
      avoid: '别写政治口号'
    },
    '母亲节': {
      imagery: '属于妈妈的一天，但妈妈往往是被遗忘的读者',
      seeds: [
        '妈妈先是一个读者，然后才是一个妈妈——别让"妈妈"这个身份吃掉"她"',
        '母亲节最好的礼物——是让妈妈安静地读一下午书',
        '母亲给孩子读的第一本书，往往也是她自己第一次温柔地读给自己',
        '那些陪着你长大的故事，大多是妈妈先读过',
        '母亲节可以和孩子一起做一件事——读妈妈小时候读过的那本书'
      ],
      avoid: '别写煽情营销文；别把母亲"神圣化"'
    },
    '儿童节': {
      imagery: '属于孩子的一天，但大人也在偷偷回望自己的童年',
      seeds: [
        '儿童节是大人的节——因为只有大人会真正在意它',
        '童年不是成年的准备期——它本身就是完整的',
        '我们欠孩子的，往往不是礼物，是时光——今天补一点',
        '那些塑造了我们的童书——《夏洛的网》《小王子》《窗边的小豆豆》……',
        '儿童节最好的礼物，是今天晚上的故事时间长一点'
      ],
      avoid: '别写"祝宝宝节日快乐"的朋友圈口吻'
    },
    '父亲节': {
      imagery: '属于爸爸的一天，但爸爸往往不太会表达',
      seeds: [
        '父亲的爱常常是沉默的——而书是沉默的爱最好的载体',
        '爸爸读故事的声音和妈妈不一样——孩子需要两种声音',
        '我们成年后读的书，很多是在找"父亲"——不是那个具体的人，是一种笃定感',
        '父亲节可以做一件事——和爸爸聊一本书，哪怕只聊三分钟',
        '一个愿意陪孩子读书的父亲，比"成功"的父亲值钱得多'
      ],
      avoid: '别写父爱"如山"的套话'
    },
    '端午节': {
      imagery: '粽子、艾草、龙舟、初夏的雨、屈原',
      seeds: [
        '端午节最动人的地方——两千多年前一个人的故事，今天还在被讲',
        '民俗是会流失的——除非一代代人愿意讲给下一代',
        '包粽子那半个下午，是很难得的家庭阅读场景',
        '给孩子讲屈原——不是讲悲情，是讲"一个人愿意为一件事投入全部"',
        '端午节的仪式感——告诉孩子"我们是从哪里来的"'
      ],
      avoid: '别只写"吃粽子"；别成为百科词条'
    },
    '七夕节': {
      imagery: '牛郎织女、夏末的夜晚、不见得浪漫的真实生活',
      seeds: [
        '中国式的浪漫——隔着一年365天，只在一天相见，反而更动人',
        '古老故事里藏着的温柔——值得一代代讲下去',
        '爱与等待——现代人越来越缺的那种能力',
        '七夕不同于2.14——它不要求你买什么，它要求你记得',
        '给孩子讲牛郎织女——不要把它讲成爱情故事，讲成一个关于星星的故事'
      ],
      avoid: '别和西方情人节混为一谈；别写"秀恩爱"文案'
    },
    '中秋节': {
      imagery: '月圆、家人、月饼、秋夜的凉',
      seeds: [
        '"圆"的反面不是"缺"，是"没人陪"',
        '离家的人的中秋——可能会读一本家乡作家的书',
        '月亮这个意象，在中国人的书里出现过多少次——古今中外的月亮不一样',
        '"海上生明月，天涯共此时"——这句话对孩子来说，是一次关于"共享"的启蒙',
        '中秋夜适合关掉电视，和孩子一起在阳台上坐一会儿——不用说话'
      ],
      avoid: '别写"但愿人长久"的套用'
    },
    '教师节': {
      imagery: '感谢一个把时间花在你身上的人的日子',
      seeds: [
        '每个人心里都有一个"那位老师"——他/她可能不知道自己被这样记着',
        '最好的老师，不教知识，教"怎么学"',
        '书是一种安静的老师——它不会催你，但它一直在',
        '教师节可以和孩子一起——回忆他最喜欢的那位老师，然后认真感谢一下',
        '父母其实是孩子的第一任老师——你今天读的书，明天就是他的课堂'
      ],
      avoid: '别写"祝老师桃李满天下"的模板'
    },
    '国庆节': {
      imagery: '长假、出行、初秋、人与人的聚散',
      seeds: [
        '长假的第一天，很多人会莫名想读书——那是忙碌之后的反弹',
        '国庆假期，可以是一次家庭阅读周',
        '远行的路上，一本书比手机好——因为它不需要信号',
        '"国"是抽象的，但书里的每一个人物都是具体的——孩子从具体开始认识世界',
        '国庆七天，留一天什么都不做——只读书'
      ],
      avoid: '别写政治宣传腔'
    },
    '重阳节': {
      imagery: '登高、菊花、深秋、敬老',
      seeds: [
        '重阳节适合想想家里的长辈——他们当年读过什么书',
        '长辈的阅读史，其实就是一段家史',
        '老人讲故事的速度——是这个时代最珍贵的节奏',
        '重阳节可以做一件事——让孩子采访一次爷爷或奶奶',
        '"九九"是长寿——而真正的长寿是故事被讲下去'
      ],
      avoid: '别写得老气横秋'
    },
    '万圣节': {
      imagery: '南瓜灯、糖果、装扮、一点点小害怕的快乐',
      seeds: [
        '孩子需要有"怕"的体验——但要在安全的范围里，比如书里',
        '童书里的怪物，其实都是孩子情绪的具象化',
        '万圣节最好的礼物，是一本不太吓人但有点悬念的绘本',
        '"害怕"不是要消除的情绪——是要理解的情绪',
        '装扮是孩子的一种叙事——他在说：我想成为谁'
      ],
      avoid: '别写恐怖套路；别反对这个节'
    },
    '感恩节': {
      imagery: '秋深了，一家人围坐，谢谢这一年',
      seeds: [
        '感恩不是仪式——是一种每天都可以做一点的练习',
        '今天可以想一想：这一年里，有哪本书帮过你',
        '给孩子讲感恩——不要讲"要感谢父母"，讲"我感谢你让我成为现在的样子"',
        '书本身就是一种被分享的善意——有人写下来，有人翻译，有人送到你手里',
        '感恩节适合读一本关于家人的书——《小海螺和大鲸鱼》也算'
      ],
      avoid: '别写"感恩一切"的鸡汤'
    },
    '腊八节': {
      imagery: '腊八粥、年的前奏、冬至之后的收束',
      seeds: [
        '腊八是年的"前奏"——从这一天开始，日子有了年味',
        '一碗热粥的仪式感——现代人越来越需要这种',
        '民俗的核心是"慢"——慢慢熬、慢慢等、慢慢讲',
        '腊八给孩子读一本关于食物的绘本——食物是童年最稳的记忆',
        '八种豆子熬成一锅粥——这本身就是一个关于"包容"的故事'
      ],
      avoid: '别只说"喝粥"'
    },
    '圣诞节': {
      imagery: '冬夜、灯光、家庭、一个异文化的节日被中国人温柔消化',
      seeds: [
        '圣诞节最好的礼物，其实是故事——《圣诞前夜》被读了两百年',
        '中国家庭过圣诞，有一种可爱的不伦不类——这也挺好',
        '冬夜、灯、一家人——不管是什么节，这三件事就够了',
        '圣诞主题的童书，是冬天里最暖的存在',
        '给孩子一个不消费的圣诞——比如读一本书，讲一个故事'
      ],
      avoid: '别假装我们是西方家庭；别过度商业化'
    }
  };

  // 星期节奏：一周里每天的微妙气质
  const weekdayMood: Record<string, string> = {
    '周一': '新一周的启动日，适合轻一点的开场、一点不刻意的动力',
    '周二': '节奏刚进入，适合平静、稳住',
    '周三': '一周的半山腰，适合一点自我确认',
    '周四': '距离周末不远了，能量开始松动',
    '周五': '收束与期待并存，适合带点松弛和回望',
    '周六': '自己的时间回来了，节奏放慢',
    '周日': '一周的收尾与下一周的序曲，适合收拢、沉淀'
  };

  // 组装"与日期的联结"段落：根据是否有节日/节气，决定绑定配额
  const hasFestival = !!context.festival && !!festivalHooks[context.festival];
  const hasSolarTerm = !!context.solarTerm && !!solarTermHooks[context.solarTerm];

  // 打乱数组顺序的小工具（Fisher-Yates），用来让 5 段对应的种子顺序随机
  const shuffle = <T>(arr: T[]): T[] => {
    const a = [...arr];
    for (let i = a.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [a[i], a[j]] = [a[j], a[i]];
    }
    return a;
  };

  let dateBindingSection = '';
  if (hasFestival) {
    const fh = festivalHooks[context.festival];
    const shuffledSeeds = shuffle(fh.seeds);
    dateBindingSection = `
**今天是 ${context.festival}**。今天生成的整组文案都围绕这个节日展开——节日就是今天的主题，不是点缀。

**画面与氛围**：${fh.imagery}

**5 段文案的分工**（每段对应一个不同的切入点，**不要让 5 段都讲同一件事**）：
${shuffledSeeds.map((s, i) => `第 ${i + 1} 段的方向：${s}`).join('\n')}

**一定要避免**：${fh.avoid}

**硬性要求**：
- 5 段文案**全部**围绕"${context.festival}"展开——每一段读完都要让读者明确感觉到"这段是为这个节日写的"。
- 但 5 段的**切入角度要明显不同**：有的可以是场景、有的是主题、有的是一个反思、有的是一句话的意象——别全是同一种语气、同一个观点。
- 节日和阅读/成长/童年的主线要**自然融合**——节日是背景，阅读/成长是主角，不能只写节日祝福。
- 不能把节日名当作挂件塞在句末——节日要成为那段文字的"骨架"，不是"装饰"。`;
  } else if (hasSolarTerm) {
    const sh = solarTermHooks[context.solarTerm];
    const shuffledSeeds = shuffle(sh.seeds);
    dateBindingSection = `
**今天是 ${context.solarTerm} 节气**。今天生成的整组文案都围绕这个节气展开——节气就是今天的底色。

**节气意象**：${sh.imagery}

**5 段文案的分工**（每段对应一个不同的切入点，**不要让 5 段都讲同一件事**）：
${shuffledSeeds.map((s, i) => `第 ${i + 1} 段的方向：${s}`).join('\n')}

**硬性要求**：
- 5 段文案**全部**要让节气的感觉渗透进来——不必每段都说"${context.solarTerm}"这个词，但每段都要能让读者感到"这是${context.solarTerm}时节才会写出来的文字"。
- 节气可以是直接的场景（温度、光线、气味），也可以是间接的心境（节气对应的生活节奏、内心状态）——5 段要**混着用**，别全是同一种写法。
- 节气和阅读/成长的思考要**自然映照**——节气是氛围，阅读/成长是主线，两者互相成就。
- 别像在写节气介绍——要像在这个节气里读书的人写下的一段话。`;
  } else {
    dateBindingSection = `
**今天没有节日或节气**，日期联结的主要抓手是**季节与星期**。

**季节的感觉（${context.season}）**：${seasonImg.feel}  
**可以用的意象**：${seasonImg.details}  
**${seasonImg.avoid}**

**今天是 ${context.weekday}**：${weekdayMood[context.weekday] || ''}

**硬性要求**：5 段文案中至少有 **1-2 段**让季节的意象自然出现——不必点名"春天/夏天"这种词，而是让那些只属于当下季节的细节（光线、温度、声音、气味）渗进文字。如果合适，可以让 1 段微妙呼应${context.weekday}的节奏感。`;
  }

  const json_structure = {
    morning: `"morning_copies": ["...","...","...","...","..."]`
  };

  // 第二部分：当有节日/节气时，主题维度降级为"观察角度的参考"，不再主导
  const hasDateAnchor = hasFestival || hasSolarTerm;
  const anchorLabel = hasFestival ? context.festival : (hasSolarTerm ? `${context.solarTerm} 节气` : '');

  const themeSection = hasDateAnchor
    ? `
## 第二部分：观察节日/节气的透镜（辅助参考，不是主角）

今天的主角是"**${anchorLabel}**"。第一部分已经给了 5 段的具体方向，这一部分只是提供一个**可选的观察角度**——你可以用这个视角去看${anchorLabel}，让文字更深一层。

**可参考的思考维度**：${randomTheme.dimension}  
${randomTheme.description}

**一个可参考的认知**（不必直接套用，只是给你一个"看${anchorLabel}的眼光"）：  
${randomSeed}

**写作角度**：${angle.angle}  
${angle.hint}

**注意**：这一部分是**辅助**，不要让它喧宾夺主。5 段文案的核心仍然是${anchorLabel}本身，这个主题维度只是帮你写得有层次。`
    : `
## 第二部分：内容维度与思考素材

**这次的内容维度**：${randomTheme.dimension}  
${randomTheme.description}

**灵感种子**（围绕这个方向写，但不要照搬原文，用自己的方式重新表达）：  
${randomSeed}

**同一维度下的其他思考角度（5段文案请覆盖不同角度，别围着一个点转）**：
${randomTheme.seeds.map(s => '- ' + s).join('\n')}

**写作角度**：${angle.angle}  
${angle.hint}`;

  const task_description = `
**## 创作5段早安文案**

**今天是**：${displayDate} ${context.weekday}  
**季节**：${context.season}${context.solarTerm ? `,${context.solarTerm}` : ''}${context.festival ? `,${context.festival}` : ''}

---

## 第一部分：与日期的联结（本次创作的核心约束）
${dateBindingSection}

---
${themeSection}

---

## 第三部分：基本要求

- 5段文案，每段都要有点不一样
- 别出现"早安"、"清晨"、"早餐"这些太直白的词
- 每段要有一个**实在的观点或洞察**，不能只是"读书真好"这种空泛的感慨
- 可以引用名人的话（但要真实），也可以提出一个让人停下来想想的观点
- 和阅读、教育、成长、童年有关，但**不讲具体的阅读方法**（那是幼儿段/小学段的事）
- 一句一行排版
- 大约2-4句话一段

**写作心法**：
- **有观点**：每段话的核心是一个具体的认知或洞察，而不是一段优美的废话
- **不空泛**：避免"阅读让人生更美好"这种正确但没有信息量的话。要说就说具体一点——"美好"在哪？怎么"美好"？
- **有画面**：可以有场景感，但场景是为观点服务的，不是为了"文艺"
- **通用视角**：严禁编造"我家孩子"的假故事
- **日期绑定不是装饰**：第一部分要求的"绑定段落"，必须能让读者感到"这是为今天写的"，而不是在普适文字的末尾加一个节日名
`;

  let output_format = `{ ${json_structure.morning} }`;

  if (regenerate_column) {
    if (regenerate_column !== 'morning') {
      throw new Error('早安语模式只支持重新生成早安语内容');
    }
  }

  return `直接返回纯JSON，不要任何额外文字或markdown标记。

# 角色定位：阅读陪伴者
你不是高高在上的教育专家，也不是虚构的"过来人家长"。
你是一位懂阅读、懂孩子、温暖而专业的陪伴者。你站在家长身边，用平等的视角分享阅读的美好与方法。

# 核心原则：真实与真诚
1. **绝对禁止虚构经历**：严禁使用"我家娃"、"我以前"、"我发现"、"我们家"等第一人称叙述虚构的生活场景。不要扮演一个不存在的家长。
2. **对话视角**：
   - 主要使用第二人称"你"（家长）和"孩子"。
   - 或者使用客观的第三人称视角描述普遍规律。
   - 示例："当孩子读不下去时，你可以试试..."（推荐） vs "我家孩子读不下去时，我用了..."（禁止）

# 语言风格指南
1. **专业但不晦涩**：把专业的阅读理论翻译成家长一听就懂的大白话。
2. **建议而非指令**：
   - 避免："你应该"、"必须"、"切记"。
   - 提倡："不妨试试"、"或许可以"、"你会发现"。
3. **温暖而有洞见**：语言要柔软，但要有力量。指出问题的同时，给出充满希望的解决方案。

${task_description}

# 输出格式
${output_format}

直接返回JSON，别加代码块。`;
}

// 调用aihubmix平台的Gemini API生成早安语
async function callAihubmixGeminiMorning(
  prompt: string,
  regenerate_column?: 'morning' | 'toddler' | 'primary'
): Promise<Partial<GeneratedContent>> {
  const requestBody = {
    model: 'gemini-2.5-pro',
    messages: [
      {
        role: 'user',
        content: prompt
      }
    ],
    temperature: 0.7, // 降低温度值提高稳定性
    max_tokens: 8000, // 增大 token 限制，避免 prompt 较长时（例如节日场景）AI 输出被截断导致 JSON 解析失败
    stream: false,
    top_p: 0.8,
    frequency_penalty: 0.1,
    presence_penalty: 0.1
  };
  
  const response = await fetch(AIHUBMIX_API_URL, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${process.env.GEMINI_API_KEY}`,
    },
    body: JSON.stringify(requestBody),
  });

  if (!response.ok) {
    const errorData = await response.text();
    console.error('aihubmix API调用失败:', response.status, errorData);
    throw new Error(`API调用失败: ${response.status} ${response.statusText} - ${errorData}`);
  }

  const data = await response.json();

  if (!data.choices || !data.choices[0] || !data.choices[0].message) {
    console.error('API返回数据格式错误:', data);
    throw new Error('API返回数据格式错误');
  }

  const content = data.choices[0].message.content;
  
  // 简单清理并解析JSON - 采用多策略尝试
  const parsed = parseAIResponse(content, regenerate_column);
  
  if (!parsed) {
    console.error('所有解析策略都失败了');
    throw new Error('AI返回内容格式错误，无法解析JSON');
  }
  
  return parsed;
}

// 新增：简单的JSON解析函数，采用多种策略
function parseAIResponse(content: string, regenerate_column?: string): any {
  // 策略1：直接解析（AI返回了正确的JSON）
  try {
    const parsed = JSON.parse(content);
    if (validateParsedContent(parsed, regenerate_column)) {
      return parsed;
    }
  } catch (e) {
    // 继续尝试其他策略
  }

  // 策略2：提取JSON并直接解析
  try {
    let cleaned = content.trim();
    
    // 移除markdown代码块
    cleaned = cleaned.replace(/```json\s*/gi, '').replace(/```\s*/g, '');
    
    // 提取 { } 之间的内容
    const start = cleaned.indexOf('{');
    const end = cleaned.lastIndexOf('}');
    if (start !== -1 && end !== -1 && end > start) {
      cleaned = cleaned.substring(start, end + 1);
      const parsed = JSON.parse(cleaned);
      if (validateParsedContent(parsed, regenerate_column)) {
        return parsed;
      }
    }
  } catch (e) {
    // 继续尝试其他策略
  }

  // 策略3：转义所有换行符后解析（处理未转义的换行符）
  try {
    let cleaned = content.trim();
    cleaned = cleaned.replace(/```json\s*/gi, '').replace(/```\s*/g, '');
    
    const start = cleaned.indexOf('{');
    const end = cleaned.lastIndexOf('}');
    if (start !== -1 && end !== -1 && end > start) {
      cleaned = cleaned.substring(start, end + 1);
      
      // 简单粗暴：将所有真实换行符替换为\n（可能不完美但通常有效）
      cleaned = cleaned
        .replace(/\r\n/g, '\\n')
        .replace(/\r/g, '\\n')
        .replace(/\n/g, '\\n');
      
      const parsed = JSON.parse(cleaned);
      if (validateParsedContent(parsed, regenerate_column)) {
        return parsed;
      }
    }
  } catch (e) {
    // 继续尝试其他策略
  }

  // 策略4：使用容错解析
  try {
    const partial = extractPartialContent(content);
    if (partial && validateParsedContent(partial, regenerate_column)) {
      return partial;
    }
  } catch (e) {
    // 所有策略都失败
  }

  return null;
}

// 新增：验证解析后的内容是否符合要求
function validateParsedContent(parsed: any, regenerate_column?: string): boolean {
  if (!parsed) return false;
  
  const validators: Record<string, (p: any) => boolean> = {
    morning: (p: any) => p.morning_copies && Array.isArray(p.morning_copies) && p.morning_copies.length === 5,
    toddler: (p: any) => p.toddler_copies && Array.isArray(p.toddler_copies) && p.toddler_copies.length === 4,
    primary: (p: any) => p.primary_copies && Array.isArray(p.primary_copies) && p.primary_copies.length === 4,
  };

  if (regenerate_column) {
    return validators[regenerate_column]?.(parsed) || false;
  } else {
    // 早安语模式，只需要 morning_copies
    return validators.morning(parsed);
  }
}

// 尝试从不完整的响应中提取部分内容（简化版）
function extractPartialContent(content: string): GeneratedContent | null {
  try {
    let fixed = content.trim();
    
    // 移除markdown
    fixed = fixed.replace(/```json\s*/gi, '').replace(/```\s*/g, '');
    
    // 提取JSON
    const start = fixed.indexOf('{');
    const end = fixed.lastIndexOf('}');
    if (start === -1 || end === -1 || end <= start) return null;
    
    fixed = fixed.substring(start, end + 1);
    
    // 转义换行符
    fixed = fixed.replace(/\r\n/g, '\\n').replace(/\r/g, '\\n').replace(/\n/g, '\\n');
    
    return JSON.parse(fixed);
  } catch (e) {
    return null;
  }
}

// 调用aihubmix平台的Gemini API生成幼儿段/小学段内容
async function callAihubmixGeminiSegment(prompt: string): Promise<SegmentContent> {
  const requestBody = {
    model: 'gemini-2.5-pro',
    messages: [
      {
        role: 'user',
        content: prompt
      }
    ],
    temperature: 0.7,
    max_tokens: 8000, // 增大 token 限制，避免长 prompt（如绘本语言整段摘录）输出被截断
    stream: false,
    top_p: 0.8,
    frequency_penalty: 0.1,
    presence_penalty: 0.1
  };
  
  const response = await fetch(AIHUBMIX_API_URL, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${process.env.GEMINI_API_KEY}`,
    },
    body: JSON.stringify(requestBody),
  });

  if (!response.ok) {
    const errorData = await response.text();
    console.error('段落生成API调用失败:', response.status, errorData);
    throw new Error(`API调用失败: ${response.status} ${response.statusText} - ${errorData}`);
  }

  const data = await response.json();

  if (!data.choices || !data.choices[0] || !data.choices[0].message) {
    console.error('段落生成API返回数据格式错误:', data);
    throw new Error('API返回数据格式错误');
  }

  const content = data.choices[0].message.content;
  
  // 检查内容是否为空
  if (!content || content.trim() === '') {
    console.error('API返回空内容:', data);
    throw new Error('API返回空内容，请检查API配置和网络连接');
  }
  
  // 使用简化的解析策略
  const parsed = parseSegmentResponse(content);
  
  if (!parsed) {
    console.error('所有解析策略都失败了，原始内容:');
    console.error('--- 原始内容开始 ---');
    console.error(content.substring(0, 2000)); // 只打印前2000字符避免日志过长
    console.error('--- 原始内容结束 ---');
    throw new Error('AI返回内容格式错误，无法解析JSON');
  }
  
  // 验证必需字段
  if (!parsed.copies || !Array.isArray(parsed.copies) || typeof parsed.quote_index !== 'number') {
    throw new Error('JSON缺少必需字段或格式错误');
  }

  // quote_index 可以是 -1（表示不包含名人名言）或有效的索引位置
  if (parsed.quote_index !== -1 && (parsed.quote_index < 0 || parsed.quote_index >= parsed.copies.length)) {
    throw new Error('quote_index超出范围');
  }
  
  return parsed as SegmentContent;
}

// 新增：段落内容的简单解析函数（增强版，处理绘本语言等复杂格式）
function parseSegmentResponse(content: string): any {
  // 策略1：直接解析
  try {
    return JSON.parse(content);
  } catch (e) {
    // 继续尝试其他策略
  }

  // 策略2：提取并解析
  try {
    let cleaned = content.trim().replace(/```json\s*/gi, '').replace(/```\s*/g, '');
    const start = cleaned.indexOf('{');
    const end = cleaned.lastIndexOf('}');
    if (start !== -1 && end !== -1 && end > start) {
      cleaned = cleaned.substring(start, end + 1);
      return JSON.parse(cleaned);
    }
  } catch (e) {
    // 继续尝试其他策略
  }

  // 策略3：转义换行符（基础版）
  try {
    let cleaned = content.trim().replace(/```json\s*/gi, '').replace(/```\s*/g, '');
    const start = cleaned.indexOf('{');
    const end = cleaned.lastIndexOf('}');
    if (start !== -1 && end !== -1 && end > start) {
      cleaned = cleaned.substring(start, end + 1);
      cleaned = cleaned.replace(/\r\n/g, '\\n').replace(/\r/g, '\\n').replace(/\n/g, '\\n');
      return JSON.parse(cleaned);
    }
  } catch (e) {
    // 继续尝试其他策略
  }

  // 策略4：智能处理字符串内部的换行符（专门处理绘本语言等复杂内容）
  try {
    let cleaned = content.trim().replace(/```json\s*/gi, '').replace(/```\s*/g, '');
    const start = cleaned.indexOf('{');
    const end = cleaned.lastIndexOf('}');
    if (start !== -1 && end !== -1 && end > start) {
      cleaned = cleaned.substring(start, end + 1);
      
      // 只转义双引号内的换行符，保留 JSON 结构的换行
      let inString = false;
      let escaped = false;
      let result = '';
      
      for (let i = 0; i < cleaned.length; i++) {
        const char = cleaned[i];
        
        if (escaped) {
          result += char;
          escaped = false;
          continue;
        }
        
        if (char === '\\') {
          escaped = true;
          result += char;
          continue;
        }
        
        if (char === '"') {
          inString = !inString;
          result += char;
          continue;
        }
        
        // 在字符串内部，将真实换行符转义
        if (inString && (char === '\n' || char === '\r')) {
          if (char === '\r' && cleaned[i + 1] === '\n') {
            result += '\\n';
            i++; // 跳过 \n
          } else {
            result += '\\n';
          }
          continue;
        }
        
        result += char;
      }
      
      return JSON.parse(result);
    }
  } catch (e) {
    // 继续尝试其他策略
  }

  // 策略5：使用正则提取 copies 数组（最后的兜底方案）
  try {
    let cleaned = content.trim().replace(/```json\s*/gi, '').replace(/```\s*/g, '');
    
    // 尝试匹配 copies 数组的内容
    const copiesMatch = cleaned.match(/"copies"\s*:\s*\[([\s\S]*?)\]\s*[,}]/);
    const quoteIndexMatch = cleaned.match(/"quote_index"\s*:\s*(-?\d+)/);
    
    if (copiesMatch) {
      // 提取数组内容并手动解析
      const arrayContent = copiesMatch[1];
      const copies: string[] = [];
      
      // 匹配每个字符串元素
      const stringPattern = /"((?:[^"\\]|\\.)*)"/g;
      let match;
      while ((match = stringPattern.exec(arrayContent)) !== null) {
        // 处理转义字符
        let str = match[1]
          .replace(/\\n/g, '\n')
          .replace(/\\"/g, '"')
          .replace(/\\\\/g, '\\');
        copies.push(str);
      }
      
      if (copies.length > 0) {
        return {
          copies,
          quote_index: quoteIndexMatch ? parseInt(quoteIndexMatch[1]) : -1
        };
      }
    }
  } catch (e) {
    // 所有策略都失败
  }

  return null;
}


// 调试端点：GET /api/generate?debug=YYYY-MM-DD
// 功能：预览某个日期会被识别成什么节日/节气/季节/星期，以及将要发送给 AI 的完整 prompt
// 用法示例：
//   /api/generate?debug=2026-04-23   （世界读书日）
//   /api/generate?debug=2026-05-10   （母亲节，浮动日期）
//   /api/generate?debug=2026-06-21   （夏至）
//   /api/generate?debug=2026-03-15   （普通日期，走季节/星期兜底）
export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);
    const debugDate = searchParams.get('debug') || searchParams.get('date');

    if (!debugDate) {
      return NextResponse.json({
        success: false,
        error: '请提供 date 参数，例如 /api/generate?debug=2026-04-23',
        usage: {
          examples: [
            '/api/generate?debug=2026-04-23 （世界读书日）',
            '/api/generate?debug=2026-05-10 （母亲节，浮动日期）',
            '/api/generate?debug=2026-06-21 （夏至）',
            '/api/generate?debug=2026-03-15 （普通日期）'
          ]
        }
      }, { status: 400 });
    }

    // 解析日期上下文
    const context = getDateContext(debugDate);

    // 判断将走哪条路径（与 createMorningPrompt 里的逻辑对齐）
    let bindingMode: 'festival' | 'solarTerm' | 'default';
    let anchor: string;
    if (context.festival) {
      bindingMode = 'festival';
      anchor = context.festival;
    } else if (context.solarTerm) {
      bindingMode = 'solarTerm';
      anchor = context.solarTerm;
    } else {
      bindingMode = 'default';
      anchor = `${context.season} · ${context.weekday}`;
    }

    // 生成一次 prompt（包含本次随机选中的种子顺序、主题维度等）
    // 注意：每次调用都会重新随机，不代表下次生成时会一模一样
    const samplePrompt = createMorningPrompt(debugDate, context);

    return NextResponse.json({
      success: true,
      date: debugDate,
      context,
      binding: {
        mode: bindingMode,
        anchor,
        description: bindingMode === 'festival'
          ? `5 段文案将全部围绕"${anchor}"展开，每段对应 5 条种子中的一条`
          : bindingMode === 'solarTerm'
            ? `5 段文案将全部让"${anchor}"节气的感觉渗透进来`
            : `无节日/节气，以季节（${context.season}）和星期（${context.weekday}）为辅助锚点，至少 1-2 段体现季节意象`
      },
      samplePrompt,
      note: '每次调用 debug 都会重新随机抽取种子顺序和主题维度，仅代表"某一次生成时可能用到的 prompt"，不代表下次真实生成的内容。若要看种子库全貌，请直接查看 route.ts 源码中的 solarTermHooks / festivalHooks。'
    }, {
      // 让 JSON 响应里的中文换行更易读（浏览器里按 Ctrl+U 或用 JSON 格式化工具查看）
      headers: { 'Content-Type': 'application/json; charset=utf-8' }
    });

  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
      { success: false, error: `调试失败：${message}` },
      { status: 500 }
    );
  }
}

export async function POST(request: NextRequest) {
  try {
    const body: GenerateRequest = await request.json();
    const { type, dates, count, regenerate_column } = body;

    // 验证请求参数
    if (!type) {
      return NextResponse.json(
        { success: false, error: '请提供生成类型' },
        { status: 400 }
      );
    }

    if (type === 'morning' && (!dates || dates.length === 0)) {
      return NextResponse.json(
        { success: false, error: '早安语模式请提供至少一个日期' },
        { status: 400 }
      );
    }

    if ((type === 'toddler' || type === 'primary') && (!count || count < 1)) {
      return NextResponse.json(
        { success: false, error: '请提供有效的生成条数' },
        { status: 400 }
      );
    }

    if (type === 'quote' && (!count || count < 1 || !body.quoteType)) {
      return NextResponse.json(
        { success: false, error: '请提供有效的生成条数和应用场景' },
        { status: 400 }
      );
    }

    if (type === 'picturebook' && (!count || count < 1 || !body.pictureBookCategory)) {
      return NextResponse.json(
        { success: false, error: '请提供有效的生成条数和语言风格分类' },
        { status: 400 }
      );
    }
    
    if (!process.env.GEMINI_API_KEY) {
      return NextResponse.json(
        { success: false, error: 'Gemini API密钥未配置' },
        { status: 500 }
      );
    }

    // 处理名人名言生成
    if (type === 'quote') {
      try {
        const prompt = createQuotePrompt(body.quoteType!, count!);
        const content = await callAihubmixGeminiSegment(prompt);

        return NextResponse.json({
          success: true,
          content: {
            type: 'quote',
            content: content
          }
        });

      } catch (e) {
        console.error('生成名人名言时出错:', e);
        return NextResponse.json(
          { success: false, error: '生成名人名言失败' },
          { status: 500 }
        );
      }
    }

    // 处理最美绘本语言生成（带自动重试机制）
    if (type === 'picturebook') {
      const maxRetries = 3;
      let lastError: Error | null = null;
      
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          console.log(`绘本语言生成尝试 ${attempt}/${maxRetries}`);
          const prompt = createPictureBookPrompt(body.pictureBookCategory!, count!);
          const content = await callAihubmixGeminiSegment(prompt);

          return NextResponse.json({
            success: true,
            content: {
              type: 'picturebook',
              content: content
            }
          });

        } catch (e) {
          lastError = e instanceof Error ? e : new Error(String(e));
          console.error(`绘本语言生成第 ${attempt} 次尝试失败:`, lastError.message);
          
          // 如果还有重试机会，等待一小段时间后重试
          if (attempt < maxRetries) {
            await new Promise(resolve => setTimeout(resolve, 1000));
          }
        }
      }
      
      // 所有重试都失败
      console.error('绘本语言生成全部重试失败:', lastError);
      return NextResponse.json(
        { success: false, error: '生成绘本语言失败，请稍后重试' },
        { status: 500 }
      );
    }

    // 处理幼儿段/小学段生成
    if (type === 'toddler' || type === 'primary') {
      try {
        const prompt = createSegmentPrompt(type, count!);
        const content = await callAihubmixGeminiSegment(prompt);
        
        return NextResponse.json({
          success: true,
          content: {
            type: type,
            content: content
          }
        });

      } catch (e) {
        console.error(`生成${type === 'toddler' ? '幼儿段' : '小学段'}内容时出错:`, e);
        return NextResponse.json(
          { success: false, error: `生成${type === 'toddler' ? '幼儿段' : '小学段'}内容失败` },
          { status: 500 }
        );
      }
    }

    // 处理早安语分列再生逻辑
    if (type === 'morning' && regenerate_column && dates && dates.length === 1) {
      const dateString = dates[0];
      try {
        const context = getDateContext(dateString);
        const prompt = createMorningPrompt(dateString, context, regenerate_column);
        const content = await callAihubmixGeminiMorning(prompt, regenerate_column);
        
        return NextResponse.json({
          success: true,
          date: dateString,
          column: regenerate_column,
          copies: content[`${regenerate_column}_copies` as keyof GeneratedContent]
        });

      } catch (e) {
        console.error(`处理日期 ${dateString} 的再生请求时出错:`, e);
        return NextResponse.json(
          { success: false, error: `重新生成 ${dateString} 失败` },
          { status: 500 }
        );
      }
    }


    // 处理早安语批量生成
    if (type === 'morning' && dates && dates.length > 0) {
      const results: ApiResponse[] = [];
      const errors: string[] = [];

      for (const dateString of dates) {
        try {
          const context = getDateContext(dateString);
          const prompt = createMorningPrompt(dateString, context);
          
          // 调用AI生成内容
          const content = await callAihubmixGeminiMorning(prompt) as GeneratedContent;

          results.push({
            date: dateString,
            context,
            content
          });

        } catch (dateError) {
          const errorMsg = dateError instanceof Error ? dateError.message : String(dateError);
          console.error(`处理日期 ${dateString} 时出错:`, dateError);
          errors.push(`${dateString}: ${errorMsg}`);
          // 继续处理其他日期，不中断整个流程
          continue;
        }
      }

      if (results.length === 0) {
        const detailedError = errors.length > 0 
          ? `所有日期处理失败。详细错误：${errors.join('; ')}`
          : '所有日期处理失败';
        console.error('批量处理完全失败，错误列表:', errors);
        return NextResponse.json(
          { success: false, error: detailedError },
          { status: 500 }
        );
      }

      return NextResponse.json({
        success: true,
        results: results
      });
    }

    // 如果没有匹配的处理逻辑，返回错误
    return NextResponse.json(
      { success: false, error: '无效的请求参数' },
      { status: 400 }
    );

  } catch (error) {
    console.error('API调用失败:', error);
    return NextResponse.json(
      { success: false, error: '服务器内部错误' },
      { status: 500 }
    );
  }
}
