...
**结构要求：一句精辟引入 + 一条落地建议**
- 引入：用一句话点出痛点、现象或洞察（简短有力，让家长有共鸣）
- 建议：紧接着给一个具体的、可操作的方法或策略
- 引入和建议之间要有逻辑关联，不能生硬拼接

**引入方式要多样**（不要每段都用提问开头）：
- 可以是一句观察："很多孩子读书只挑自己熟悉的类型。"
- 可以是一个类比："选书就像交朋友，总待在舒适圈，就遇不到惊喜。"
- 可以是一句反常识："读得慢的孩子，未必是阅读能力差。"
- 可以是一个场景："书店里，孩子径直走向漫画区，你叹了口气。"
